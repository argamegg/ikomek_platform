import i18n from "i18next";
import { initReactI18next } from "react-i18next";
import { session } from "../lib/session";

const resources = {
  en: {
    translation: {
      brand: {
        name: "iKOMEK 109",
        tagline: "Smart city platform",
      },
      common: {
        loading: "Loading",
        retry: "Retry",
        back: "Back",
        continue: "Continue",
        save: "Save",
        done: "Done",
        cancel: "Cancel",
        close: "Close",
        confirm: "Confirm",
        create: "Create",
        update: "Update",
        edit: "Edit",
        delete: "Delete",
        send: "Send",
        logout: "Log out",
        login: "Sign in",
        register: "Create account",
        profile: "Profile",
        dashboard: "Dashboard",
        map: "Map",
        requests: "Requests",
        news: "News",
        operator: "Operations",
        admin: "Admin",
        details: "Details",
        chat: "Chat",
        language: "Language",
        search: "Search",
        searchPlaceholder: "Search by title, address or request ID",
        submitRequest: "New request",
        submitRequestShort: "Request",
        empty: "No data yet",
        public: "Public",
        private: "Private",
        attachment: "Attachment",
        attachImage: "Attach image",
        zoomIn: "Zoom in",
        zoomOut: "Zoom out",
        locateMe: "Locate me",
      },
      priority: {
        unset: "Unset",
        low: "Low",
        medium: "Medium",
        high: "High",
      },
      nav: {
        home: "Home",
        cabinet: "My Cabinet",
        dashboard: "Resident dashboard",
        requests: "Requests",
        newRequest: "Submit issue",
        news: "News & alerts",
        map: "City map",
        profile: "Profile",
        settings: "Settings",
        operator: "Operator board",
        admin: "Admin studio",
        auth: "Authentication",
        sections: {
          main: "Main menu",
          personal: "Personal account",
          workplace: "Workplace",
          management: "Management",
          system: "System",
        },
      },
      shell: {
        account: "Account",
        status: "Shared platform connection",
        connected: "Live against the existing FastAPI platform",
        guest: "Guest mode",
        userProfile: "User profile",
        welcome: "One platform for residents, operators, and the city team.",
        preparing: "Preparing your workspace...",
        apiError: "API error:",
        toggleNavigation: "Toggle navigation",
        closeNavigation: "Close navigation",
        expandSidebar: "Expand sidebar",
        collapseSidebar: "Collapse sidebar",
      },
      roles: {
        admin: "Administrator",
        operator: "Operator",
        citizen: "Resident",
        executor: "Executor",
      },
      home: {
        badge: "Unified city operations platform",
        title: "One city service center for iKOMEK 109.",
        description:
          "Residents, operators, and managers connect through one shared city platform, one live process, one chat and one city map.",
        primary: "Report an issue",
        secondary: "View the map",
        heroTrust: {
          queue: "One live request queue",
          roles: "Role-based actions",
          map: "Public city map",
        },
        stats: {
          requests: "Public city reports on the map",
          alerts: "Live city alerts",
          inProgress: "Requests in progress",
          categories: "Service categories",
        },
        overview: {
          eyebrow: "Platform overview",
          title: "One platform powering requests, response and city coordination.",
          description:
            "A shared city workspace for residents, operators, and administrators.",
          boardEyebrow: "Live workload",
          boardTitle: "What is happening in the city right now",
          narrative:
            "The homepage now shows the operational picture directly: how many reports are waiting, how many are already being handled, and how many have been resolved.",
          visibleIssues: "Live city issue stream",
          pendingMetric: "Waiting for review",
          inProgressMetric: "In operator work",
          closedMetric: "Resolved requests",
          categoriesMetric: "Service routes",
          cardOneEyebrow: "Shared data",
          cardOneTitle: "All city processes in one shared system.",
          cardOneBody:
            "Everyone sees the same requests, statuses and map context in one connected system.",
          cardTwoEyebrow: "Responsive operations",
          cardTwoTitle: "Fast action from report to resolution.",
          cardTwoBody:
            "City teams collaborate through a shared queue instead of disconnected touchpoints.",
          cardThreeEyebrow: "Public visibility",
          cardThreeTitle: "The city map stays open for viewing.",
          cardThreeBody:
            "Guests can inspect public reports and hotspots, while contact, edits, and personal actions remain role-based.",
        },
        statsSection: {
          eyebrow: "City metrics",
          title: "Signals that come alive the moment they matter.",
        },
        preview: {
          signalLabel: "City signal",
          activeMapPoints: "{{count}} active map points",
          sharedQueue: "Web and mobile share the same live service queue.",
          categories: "Categories",
          alerts: "Alerts",
          inProgress: "In progress",
          issueStream: "Shared issue stream",
          disruptions: "Priority notices and incidents",
          inProgressDetail: "Already accepted by operators",
          syncedCategories: "Service categories synced with the platform",
          serviceRoutes: "{{count}} service routes ready",
          liveIssuePulse: "Live city pulse",
          mapPoints: "{{count}} points on the map",
          alertsCount: "{{count}} alerts",
          inProgressCount: "{{count}} in progress",
          selectedIssue: "Selected issue",
          publicRequest: "Public request",
          requestId: "#{{id}}",
          closeSelected: "Close selected issue",
          openDetails: "Open details",
          signInToAct: "Sign in to act",
          authenticatedHint: "Open the full request to view the timeline, chat, and available role actions.",
          guestHint: "Guest mode is view-only: sign in to create, contact, or manage requests.",
        },
        newsPreview: {
          eyebrow: "City updates",
          title: "Useful city updates without losing the request flow.",
          description:
            "Search official announcements, service changes, and city posts while the main page stays focused on requests.",
          locationFallback: "Astana",
        },
        alertsStory: {
          eyebrow: "Public awareness",
          title: "Alerts tied directly to nearby action.",
          body: "Residents see disruptions and notices in the same interface where requests are created.",
          landingMetric: "Editorial updates and city alerts on the landing page",
        },
        severity: {
          critical: "Critical",
          warning: "Warning",
          information: "Information",
          high: "High",
          medium: "Medium",
          low: "Low",
          unknown: "Update",
        },
        categoriesSection: {
          eyebrow: "What you can report",
          title: "Choose the city issue that best matches what needs to be fixed.",
          description:
            "Each category helps residents understand what can be reported and what city services usually resolve through iKOMEK 109.",
          modal: {
            close: "Close category details",
            problemType: "Problem type",
            detailsTitle: "What gets resolved",
            examplesTitle: "Examples",
            whenToUseTitle: "When to use this category",
          },
          labels: {
            electricity: "Electricity",
            water: "Water Supply",
            heating: "Heating",
            public_order: "Public Order",
            sewage: "Sewage",
            waste: "Waste",
            roads: "Roads",
            other: "Other",
          },
          cards: {
            electricity: {
              description: "Lighting outages, power cuts, damaged lines, and electrical failures.",
              details:
                "Use this category for outages, dangerous exposed cables, damaged transformers, or broken street lighting that requires an electrical response.",
              example: "Streetlights, cable faults, transformer issues.",
              whenToUse:
                "Choose this category if the main issue is power, lighting, or electrical infrastructure.",
            },
            water: {
              description: "No water, weak pressure, leaks, and supply interruptions.",
              details:
                "Use this category when homes, buildings, or outdoor infrastructure face water supply interruptions, visible leaks, or unstable pressure.",
              example: "Building supply issues, burst pipes, emergency leaks.",
              whenToUse:
                "Choose this category if the main problem is drinking water supply, pressure, or leakage.",
            },
            heating: {
              description: "Heating problems in homes, cold radiators, and service disruptions.",
              details:
                "Use this category for cold apartments, unstable heating service, radiator problems, or other heating-system failures during the season.",
              example: "No heat, uneven heating, recurring interruptions.",
              whenToUse:
                "Choose this category if the issue affects indoor heat supply or building heating systems.",
            },
            public_order: {
              description: "Public disturbances, excessive noise, and unsafe or illegal activity.",
              details:
                "Use this category for recurring disturbances in shared spaces, public safety concerns, or visible behavior that requires city attention.",
              example: "Night noise, disorder, conflicts in shared public spaces.",
              whenToUse:
                "Choose this category if the issue is about safety, noise, or order in public areas.",
            },
            sewage: {
              description: "Sewer blockages, bad odors, flooding, and emergency failures.",
              details:
                "Use this category when wastewater systems overflow, drains are blocked, or strong sewage odors indicate a network problem.",
              example: "Blocked drains, overflowing sewage, strong smells near buildings.",
              whenToUse:
                "Choose this category if the issue is related to sewer drainage, overflow, or sanitation risks.",
            },
            waste: {
              description: "Overflowing bins, missed collection, and waste-related sanitation issues.",
              details:
                "Use this category for household waste collection problems, overflowing containers, or sanitation issues caused by delayed removal.",
              example: "Full containers, illegal dumping, delayed cleanup.",
              whenToUse:
                "Choose this category if the main issue is garbage collection, overflowing bins, or illegal dumping.",
            },
            roads: {
              description: "Potholes, damaged roads, unsafe surfaces, and marking problems.",
              details:
                "Use this category for damaged road surfaces, unsafe pavement conditions, broken curbs, or missing and faded markings.",
              example: "Road damage, broken curbs, faded lane markings.",
              whenToUse:
                "Choose this category if transport safety or road condition is the main problem.",
            },
            other: {
              description: "Other city problems that do not fit the main reporting categories.",
              details:
                "Use this category when the issue still needs city attention but does not clearly belong to electricity, water, roads, waste, or another main service group.",
              example: "Use this if your issue needs attention but belongs elsewhere.",
              whenToUse:
                "Choose this category if none of the main categories describe your issue well enough.",
            },
          },
        },
        mapPreview: {
          eyebrow: "Map preview",
          title: "A live city map preview with requests, status and next action.",
          description:
            "Inspect the public city pulse, choose a request, and move from map context to the full request record without losing the request flow.",
          panelEyebrow: "Selected signal",
          panelTitle: "Map context becomes an actionable request.",
          panelDescription:
            "The preview combines public map visibility with the operational details residents and operators need next.",
          queueTitle: "Recent public requests",
        },
        how: {
          eyebrow: "How it works",
          title: "From request to result: what happens at each step.",
          description:
            "The request flow is built around one record: each step shows who owns it, what status is visible, what data moves forward, and what outcome the resident receives.",
          statusLabel: "Visible status",
          visibleLabel: "Data and context",
          resultLabel: "Outcome",
          highlightOneValue: "1 ID",
          highlightOneLabel: "One request record from web, mobile, map, and operator panels.",
          highlightTwoValue: "Live status",
          highlightTwoLabel: "Every participant sees where the request is in the lifecycle.",
          highlightThreeValue: "Role actions",
          highlightThreeLabel: "Residents, operators, and admins see different actions on the same data.",
          stepOneTitle: "Citizen reports the issue",
          stepOneBody:
            "A resident submits a request with category, location, photos, and details from mobile or web.",
          stepOneMeta: "Category, address, media, and request ID",
          stepOneOwner: "Resident",
          stepOneStatus: "New request",
          stepOneVisible: "Category, address, media, and request ID",
          stepOneResult: "The request appears in the shared queue and on the public map when allowed.",
          stepTwoTitle: "Operators triage and assign",
          stepTwoBody:
            "Call center staff review incoming reports, update status, and coordinate the proper response path.",
          stepTwoMeta: "Status, priority, department, and internal notes",
          stepTwoOwner: "Call center operator",
          stepTwoStatus: "In review / assigned",
          stepTwoVisible: "Status, priority, department, and internal notes",
          stepTwoResult: "The request gets a responsible route instead of staying as an unstructured message.",
          stepThreeTitle: "City services resolve",
          stepThreeBody:
            "The request progresses through shared statuses with comments, notes, and supporting communication.",
          stepThreeMeta: "Operator comments and service coordination",
          stepThreeOwner: "City service team",
          stepThreeStatus: "In progress",
          stepThreeVisible: "Operator comments, service notes, and coordination history",
          stepThreeResult: "Everyone sees the same progress timeline without duplicate calls or lost context.",
          stepFourTitle: "Resident sees the result",
          stepFourBody:
            "The same person who reported the issue can track updates and close the loop with confidence.",
          stepFourMeta: "Public status, personal cabinet, and final resolution",
          stepFourOwner: "Resident and operator",
          stepFourStatus: "Resolved / closed",
          stepFourVisible: "Public status, personal cabinet, and final resolution",
          stepFourResult: "The resident understands what changed, when it changed, and where to see the record later.",
        },
        cta: {
          badge: "Report a city issue",
          title: "Turn awareness into action with one tap.",
          description:
            "The landing page guides attention toward a real request inside the shared iKOMEK flow.",
          button: "Create request",
          trust: "Shared platform, synced statuses, one live service lifecycle.",
        },
        requestStatus: {
          pending: "Pending",
          in_progress: "In progress",
          closed: "Closed",
          open: "Open",
          resolved: "Resolved",
          rejected: "Rejected",
        },
        sections: {
          pulse: "City pulse",
          pulseDescription: "Latest alerts and editorial updates from the shared news stream.",
          categories: "Popular categories",
          categoriesDescription: "Shared request taxonomy pulled from the platform catalog.",
          journey: "How the platform works",
        },
      },
      auth: {
        loginTab: "Sign in",
        registerTab: "Register",
        recoverTab: "Recover access",
        login: {
          eyebrow: "Login",
          title: "Sign in to your existing iKOMEK account",
          subtitle: "Sign in to view your profile, requests, and chat history in one place.",
        },
        register: {
          eyebrow: "Registration",
          title: "Create an account",
          subtitle: "Welcome! Please fill in your details to register.",
        },
        recovery: {
          eyebrow: "Recovery",
          title: "Recover access",
          subtitle: "Enter your details to restore access to your account.",
        },
        verification: {
          eyebrow: "Verification",
          title: "Verify your email",
          subtitle: "Enter the one-time code sent to your email to activate your iKOMEK account.",
          titleText: "Verification code",
          instructions: "Enter the 6-digit code we sent to your email address.",
          sentTo: "We sent a verification code to {{email}}.",
          code: "Verification code",
          codePlaceholder: "Enter 6-digit code",
          expiresInMinutes: "The code expires in {{minutes}} min.",
          confirm: "Verify email",
          resend: "Resend code",
          resendIn: "Resend in {{seconds}}s",
          editEmail: "Edit email",
          codeSent: "We sent a verification code to your email.",
          resent: "A new verification code was sent.",
          verified: "Email verified successfully.",
          loginBlocked: "Your account is not verified yet. Enter the code from your email to continue.",
        },
        feedback: {
          loginSuccess: "Signed in successfully",
        },
        clerk: {
          title: "Fast sign-in",
          subtitle: "Use Google for quick access, or continue with your iKOMEK email or phone below.",
          setupRequired: "Google sign-in is temporarily unavailable. You can still use your iKOMEK account below.",
          google: "Continue with Google",
          orLegacy: "or use your iKOMEK account",
          signedInTitle: "Google sign-in completed",
          signedInHint: "We are preparing your profile, requests, and chat history.",
          syncing: "Opening your account...",
          localPasswordTitle: "Create an iKOMEK password",
          localPasswordHint: "Set a separate iKOMEK password so you can also sign in with email or phone.",
          localPassword: "New password",
          localPasswordConfirm: "Repeat password",
          localPasswordSave: "Save password and continue",
          localPasswordSaved: "Password saved. You can now sign in with your iKOMEK account.",
          localPasswordFillAll: "Fill in both password fields",
          localPasswordMin: "Password must be at least 6 characters",
          localPasswordMismatch: "Passwords do not match",
          missingToken: "We could not confirm this sign-in. Please try again.",
          linkSecretRequired: "This email is already used in iKOMEK. Sign in with your email or phone, or contact support.",
          genericError: "Google sign-in failed. Please try again.",
        },
        recoverNotice: {
          title: "Password recovery is not available yet",
          body: "The server-side service does not expose a password recovery flow at the moment.",
          hint: "You can use a demo account or create a new account from this page.",
        },
        fields: {
          name: "Full name",
          email: "Email",
          emailOrPhone: "Email or phone",
          password: "Password",
          phone: "Phone",
          language: "Preferred language",
          required: "Required",
          optional: "Optional",
        },
      },
      dashboard: {
        title: "Resident command center",
        description: "Track issue progress, saved places, alerts, and personal activity in one place.",
        cards: {
          total: "All requests",
          active: "Active now",
          pending: "Awaiting action",
          saved: "Saved places",
        },
      },
      requests: {
        title: "Requests",
        eyebrow: "Unified request center",
        description: "Track all public city requests and your personal reports from one clean workspace.",
        heroLabel: "Visible now",
        publicList: "All users",
        personalList: "Personal",
        scopeLabel: "Request list scope",
        scopeAll: "All requests",
        scopeMine: "My requests",
        statusFilterLabel: "Status filter",
        statsTotal: "Total requests",
        searchPlaceholder: "Search by title, address, author, or request ID",
        listTitle: "{{count}} requests",
        listHint: "Cards are grouped by selected scope and update through search, category, priority, and status filters.",
        requestId: "Request ID",
        author: "Author",
        unknownAuthor: "Author unavailable",
        moreFilters: "More filters",
        filterCategory: "Category",
        allCategories: "All categories",
        filterPriority: "Priority",
        allPriorities: "All priorities",
        sortLabel: "Sort",
        sortNewest: "Newest first",
        sortOldest: "Oldest first",
        sortPriority: "Priority first",
        paginationLabel: "Request pages",
        paginationPrevious: "Back",
        paginationNext: "Continue",
        filtersAll: "All",
        filtersPending: "Pending",
        filtersProgress: "In progress",
        filtersClosed: "Closed",
        timeline: "Status timeline",
        attachments: "Attachments",
        empty: "No requests matched this filter.",
        statuses: {
          pending: "Pending",
          in_progress: "In Progress",
          closed: "Closed",
          open: "Open",
          resolved: "Resolved",
          rejected: "Rejected",
        },
        priorities: {
          unset: "Unset",
          low: "Low",
          medium: "Medium",
          high: "High",
        },
        attachmentTypes: {
          image: "Image",
          document: "Document",
        },
      },
      newRequest: {
        title: "Submit a new city issue",
        description: "Follow a guided city-service flow: confirm the location, choose the issue type, add context, and submit with confidence.",
        address: "Address",
        place: "Place",
        category: "Category",
        reason: "Reason",
        descriptionLabel: "Description",
        attachments: "Attachments",
        geolocate: "Use my location",
        summary: "Submission summary",
        locationRequired: "Choose coordinates inside Astana before submitting.",
        invalidCoordinates: "Enter valid latitude and longitude.",
        outOfZone: "Requests can only be created in Astana and within 15 km of the city.",
        zoneHint: "Distance to Astana center: {{distance}} km. Allowed radius: 15 km.",
        progressLabel: "Request submission progress",
        optional: "Optional",
        success: "Request submitted successfully.",
        geolocateUnavailable: "Geolocation is unavailable in this browser.",
        geolocateError: "We could not get your current location.",
        addressPlaceholder: "Street, landmark, yard, or building entrance",
        addressSearchHint: "Start typing an address in Astana. We’ll try to place the point on the map automatically.",
        addressSearchLoading: "Looking for the address and updating the map…",
        addressSearchNoResults: "We could not confidently find this address. Place the point manually on the map.",
        addressSearchError: "Address lookup is temporarily unavailable. You can still choose the point manually on the map.",
        addressSearchResolved: "Best match on the map: {{address}}",
        latitude: "Latitude",
        longitude: "Longitude",
        advancedCoordinates: "Adjust coordinates manually",
        locationMapTitle: "Check the point on the map",
        locationMapDescription: "The map shows how the system understood the address. Drag the marker or click the map if the point needs correction.",
        locationMapHint: "Click the map or drag the marker to refine the exact house, yard, or entrance.",
        locationMapFallback: "We could not determine the address precisely from this point. Move the marker or clarify the address manually.",
        locationTitle: "Where did the problem happen?",
        locationDescription: "Start with the location so the city team knows where to send the right service.",
        typeTitle: "What happened?",
        typeDescription: "Choose a category first, then specify the reason and exact place of the issue.",
        descriptionTitle: "Describe the situation",
        descriptionDescription: "A short, clear description helps operators prioritize and route the request faster.",
        descriptionPlaceholder: "What happened, when did you notice it, and what should the team pay attention to?",
        descriptionHelper: "Mention visible damage, urgency, landmarks, or access details if they matter.",
        attachmentsTitle: "Add photos and settings",
        attachmentsDescription: "Photos are optional, but they help the operator understand the issue before dispatch.",
        uploadTitle: "Drag files here or upload from your device",
        uploadDescription: "Use clear photos of the problem, surrounding area, or landmark. Images work best.",
        chooseFiles: "Choose files",
        removeFile: "Remove",
        attachmentsCount_one: "{{count}} file",
        attachmentsCount_other: "{{count}} files",
        visibilityTitle: "Visibility",
        visibilityDescription: "Choose whether this request can appear in public issue lists for other residents.",
        visibilityPublic: "Visible publicly",
        visibilityPrivate: "Only visible to you and operators",
        savedPlacesTitle: "Saved places",
        savedPlacesDescription: "Use a saved address to fill the location instantly, or continue manually.",
        saveCurrentLocation: "Save this address",
        saveCurrentLocationHint: "Add the selected point to your profile for future requests.",
        locationSaved: "Address saved to profile",
        manualLocationTitle: "Manual location",
        manualLocationDescription: "Enter the address and coordinates yourself",
        categoryTitle: "Choose category",
        categoryDescription: "Pick the service area that matches the problem most closely.",
        reasonTitle: "Choose reason",
        reasonDescription: "These reasons are filtered by the category you selected.",
        reasonPlaceholderHint: "Select a category first to reveal the matching reasons.",
        placeTitle: "Where exactly is it?",
        placeDescription: "This mirrors the mobile flow and helps route the issue to the correct crew.",
        placePlaceholder: "For example: courtyard, entrance, parking, street",
        placeHelper: "Use a quick option or type a more specific place.",
        selectCategoryFirst: "Pick a category to continue with the reason selection.",
        reviewTitle: "Review before sending",
        reviewDescription: "Make sure the request looks right. Once submitted, operators will see the same details across web and mobile.",
        reviewReady: "Everything required is filled in. The request is ready to send.",
        reviewPending: "{{count}} required fields still need attention.",
        back: "Back",
        next: "Next step",
        continueToReview: "Continue to review",
        descriptionTooShort: "Description should be at least {{count}} characters.",
        submit: "Submit request",
        summaryTitle: "Live request summary",
        summaryDescription: "This panel updates as you fill the form, so you can spot gaps before sending.",
        summaryReadyDescription: "All required fields are ready. You can submit the request now.",
        summaryStatusReady: "Ready to submit",
        summaryStatusDraft: "Needs details",
        summaryMapTitle: "Location preview",
        summaryMapEmpty: "Add valid coordinates to preview the location on the map.",
        sectionStatusReady: "Looks good",
        sectionStatusNeedsAttention: "Check location",
        missingFieldsTitle: "Still missing",
        missingFields: {
          address: "Address is required",
          location: "Valid location in the Astana zone is required",
          category: "Choose a category",
          reason: "Choose a reason",
          place: "Specify the exact place",
          description: "Add a short description",
        },
        summaryItems: {
          address: "Address",
          place: "Place",
          category: "Category",
          reason: "Reason",
          files: "Files",
          visibility: "Visibility",
          coordinates: "Coordinates",
        },
        steps: {
          location: "Location",
          type: "Type",
          description: "Description",
          attachments: "Media",
          review: "Review",
        },
        stepDescriptions: {
          location: "Confirm the location in Astana",
          type: "Choose category, reason, and exact place",
          description: "Add context for the operator",
          attachments: "Photos and visibility settings",
          review: "Check readiness before sending",
        },
        stepErrors: {
          location: "Add an address and choose a valid point inside the Astana zone.",
          type: "Choose both category and reason before continuing.",
          descriptionRequired: "Add a short description before continuing.",
          descriptionLength: "Description should be at least {{count}} characters.",
          review: "Complete all required fields before submitting the request.",
        },
        placeOptions: {
          apartment: "Apartment",
          house: "House",
          office: "Office",
          street: "Street",
          park: "Park",
          other: "Other",
        },
      },
      requestDetails: {
        overview: "Overview",
        history: "History",
        messages: "Messages",
        status: "Status",
        created: "Created",
        category: "Category",
        reason: "Reason",
        location: "Location on map",
        openChat: "Open chat",
        chatPreview: "Chat snapshot",
        noTimeline: "Status history is empty.",
        noAttachments: "No attachments yet.",
        noMessages: "No messages yet.",
        notFoundTitle: "Request not found",
        notFoundDescription: "We could not load this request from the server.",
      },
      chat: {
        title: "Request chat",
        description: "Messages stay synchronized between resident and operator in real time.",
        placeholder: "Type your message",
        message: "Message",
        sent: "Message sent",
        requestLabel: "Request",
        live: "Live",
        reconnecting: "Connecting...",
        attachment: "Attachment",
        attachMedia: "Attach media",
        emptyTitle: "No messages yet",
        emptyDescription: "Start the conversation with the operator here.",
        unavailableTitle: "Chat is unavailable",
        unavailableDescription: "Only the request author, operators, and admins can use this chat.",
      },
      notFound: {
        title: "Page not found",
        description: "This route is not part of the iKOMEK web workspace.",
      },
      aiAssistant: {
        label: "AI assistant",
        title: "AI assistant",
        subtitle: "Helps with requests, statuses, and navigation",
        greeting: "Hi! I can help you use iKOMEK 109, city requests, news, and request statuses.",
        placeholder: "Ask a question...",
        empty: "Ask how to create a request or check a status.",
        configured: "Gemini API key is not configured",
        close: "Close AI assistant",
        failed: "Could not get a response. Check the backend and AI settings.",
      },
      news: {
        title: "City news and alerts",
        description: "A polished editorial and alert experience for the existing city content stream.",
        searchPlaceholder: "Search by title or text",
        allCategories: "All categories",
        allTypes: "All types",
        periodAll: "All periods",
        periodActive: "Active",
        periodFinished: "Finished",
        periodNoPeriod: "No period",
        sortNewest: "Newest first",
        sortOldest: "Oldest first",
        noResults: "Nothing found",
        emptyFiltered: "Adjust the filters or search query and try again.",
        modalTitle: "News item",
        createdAgo: "Created {{count}} d ago",
        createdHoursAgo: "Created {{count}} h ago",
        createdJustNow: "Just created",
        period: "PERIOD",
        location: "LOCATION",
        types: {
          emergency: "Emergency Works",
          weather: "Weather Conditions",
          planned: "Planned Works",
          road: "Road Situations",
          education: "Education Department",
          events: "City Events",
        },
        categories: {
          roads: "Roads",
          utilities: "Utilities",
          transport: "Transport",
          education: "Education",
          weather: "Weather",
          improvement: "Urban Improvement",
        },
        translationStatus: {
          translated: "Translated",
          skipped: "Skipped",
          failed: "Translation failed",
        },
      },
      categories: {
        electricity: "Electricity",
        water: "Water Supply",
        heating: "Heating",
        publicOrder: "Public Order",
        sewage: "Sewage",
        waste: "Waste",
        roads: "Roads",
        streetLighting: "Street Lighting",
        other: "Other",
      },
      placeTypes: {
        apartment: "Apartment",
        house: "Private House",
        office: "Office",
        street: "Street",
        park: "Park/Square",
        entrance: "Entrance",
        yard: "Yard",
        parking: "Parking",
        other: "Other",
      },
      problemTypes: {
        electricity: {
          power_outage: "No electricity",
          voltage_issue: "Voltage fluctuations",
          damaged_cables: "Damaged cables",
          street_light: "Street light not working",
          sparks: "Sparks or fire risk",
          other: "Other electricity issue",
        },
        water: {
          no_water: "No water supply",
          low_pressure: "Low water pressure",
          pipe_leak: "Pipe leak",
          dirty_water: "Dirty or cloudy water",
          sewage: "Sewage problem",
          other: "Other water issue",
        },
        sewage: {
          blockage: "Sewage blockage",
          leak: "Sewage leak",
          odor: "Bad sewage smell",
          overflow: "Sewage overflow",
          manhole: "Open or damaged manhole",
          other: "Other sewage issue",
        },
        roads: {
          pothole: "Pothole",
          damaged_pavement: "Damaged pavement",
          road_sign: "Missing or damaged road sign",
          traffic_light: "Traffic light issue",
          road_marking: "Road marking needed",
          other: "Other road issue",
        },
        public_order: {
          noise: "Noise complaint",
          illegal_parking: "Illegal parking",
          vandalism: "Vandalism",
          abandoned_vehicle: "Abandoned vehicle",
          stray_animals: "Stray animals",
          other: "Other public order issue",
        },
        waste: {
          overflowing: "Overflowing trash bin",
          illegal_dump: "Illegal dump site",
          missed_collection: "Missed garbage collection",
          hazardous: "Hazardous waste",
          bulk_waste: "Bulk waste removal needed",
          other: "Other waste issue",
        },
        heating: {
          no_heating: "No heating",
          radiator_leak: "Radiator leak",
          cold_apartment: "Cold apartment",
          overheating: "Overheating",
          noise: "Heating system noise",
          other: "Other heating issue",
        },
        street_lighting: {
          lamp_out: "Lamp not working",
          flickering: "Flickering lamp",
          damaged_pole: "Damaged pole",
          dark_area: "Poorly lit area",
          timer: "Timer malfunction",
          other: "Other lighting issue",
        },
        other: {
          general: "General complaint",
          suggestion: "Suggestion",
          question: "Question",
          other: "Other",
        },
      },
      reasons: {
        electricity: {
          infrastructure: "Infrastructure failure",
          weather: "Weather damage",
          overload: "System overload",
          maintenance: "Needs maintenance",
          accident: "Accident or external damage",
          unknown: "Unknown cause",
        },
        water: {
          pipe_burst: "Pipe burst",
          maintenance: "Scheduled maintenance",
          infrastructure: "Aged infrastructure",
          external: "External damage",
          pressure: "Pressure issue",
          unknown: "Unknown cause",
        },
        sewage: {
          pipe: "Pipe problem",
          blockage: "Blockage",
          wear: "Pipe wear",
          overflow: "Overflow",
          odor: "Odor issue",
          infrastructure: "Aged infrastructure",
          rain: "Heavy rain",
          maintenance: "Needs maintenance",
          unknown: "Unknown cause",
        },
        roads: {
          weather: "Weather wear",
          traffic: "Traffic damage",
          construction: "Construction damage",
          age: "Wear over time",
          accident: "Accident damage",
          unknown: "Unknown cause",
        },
        public_order: {
          resident: "Resident complaint",
          safety: "Public safety concern",
          community: "Community concern",
          legal: "Legal violation",
          recurring: "Recurring issue",
          other: "Other reason",
        },
        waste: {
          schedule: "Schedule issue",
          container: "Container damage",
          illegal: "Illegal dumping",
          volume: "Increased volume",
          access: "Access problem",
          other: "Other reason",
        },
        heating: {
          boiler: "Boiler issue",
          pipe: "Pipe problem",
          system: "System failure",
          regulation: "Temperature regulation",
          maintenance: "Needs maintenance",
          unknown: "Unknown cause",
        },
        street_lighting: {
          bulb: "Bulb failure",
          electrical: "Electrical issue",
          vandalism: "Vandalism",
          timer: "Timer malfunction",
          age: "Wear over time",
          unknown: "Unknown cause",
        },
        other: {
          quality: "Service quality",
          safety: "Safety concern",
          environment: "Environmental issue",
          other: "Other",
        },
      },
      map: {
        title: "City Live Map",
        subtitle: "Astana · requests in real time",
        subtitleWithCount: "Astana · {{count}} requests in real time",
        description: "Explore clustered public issues, heat zones, and district activity.",
        all: "All requests",
        mine: "My requests",
        heatmap: "Heatmap",
        filters: {
          allRequests: "All Requests",
          myRequests: "My Requests",
          heatmap: "Heat Map",
          allCategories: "All Categories",
          allStatuses: "All Statuses",
          allPriorities: "All Priorities",
          today: "Today",
          week: "Week",
          month: "Month",
          dateRangeHelp: "Select up to 7 consecutive days",
          previousMonth: "Previous month",
          nextMonth: "Next month",
          reset: "Reset Filters",
          showFilters: "Show Filters",
          hideFilters: "Hide Filters",
        },
        analytics: {
          title: "City Analytics",
          density: "Density by District",
          categories: "Popular Categories",
          timeline: "Monthly Dynamics",
          timelineMonths: "Monthly Dynamics",
          timelineWeekdays: "Dynamics by Weekday",
          timelineHours: "Hourly Dynamics",
          timelineView: "Timeline view",
          timelineSelectedPeriod: "Selected period",
          timelineAllMonths: "All months",
          chartTotal: "Total",
          hotspots: "Top Problem Spots",
          activity: "Activity by Day and Hour",
          otherDistrict: "Other",
          seriesAll: "All requests",
          weekdays: {
            mon: "Mon",
            tue: "Tue",
            wed: "Wed",
            thu: "Thu",
            fri: "Fri",
            sat: "Sat",
            sun: "Sun",
          },
          requestsCount: "{{count}} requests",
          insufficientData: "Not enough data for analytics",
        },
      },
      profile: {
        title: "Profile & saved places",
        addLocation: "Add place",
        notifications: "Notifications",
      },
      savedLocationTypes: {
        home: "Home",
        work: "Work",
        study: "Study",
        family: "Family",
        other: "Other",
      },
      cabinet: {
        title: "My Cabinet",
        memberSince: "Member since",
        editProfile: "Edit Profile",
        logout: "Log Out",
        comingSoon: "Coming soon",
        profileEditor: {
          title: "Your details",
          displayName: "How should we address you?",
          displayNamePlaceholder: "Preferred name",
          displayNameHint: "This name appears in your reviews and comments.",
          changePhoto: "Change photo",
          removePhoto: "Remove photo",
          cropPhoto: "Crop photo",
          photoScale: "Photo size",
          photoError: "Could not prepare this photo",
          personalData: "Personal details",
          firstName: "First name",
          lastName: "Last name",
          gender: "Gender",
          birthDate: "Date of birth",
          phone: "Phone",
          language: "Interface language",
          notifications: "Receive city notifications",
          save: "Save changes",
          saved: "Profile updated",
          nameRequired: "Enter your first and last name",
          nameLettersOnly: "Use letters only, with a hyphen only between name parts",
          phoneInvalid: "Enter a Kazakhstan phone number as 11 digits starting with 7",
          birthDateInvalid: "Enter a valid birth date, not in the future",
          genders: {
            male: "M",
            female: "F",
          },
        },
        stats: {
          title: "My Statistics",
          total: "Total Requests",
          closed: "Closed",
          inProgress: "In Progress",
          pending: "Pending",
          avgDays: "Average closing time",
          days: "days",
        },
        activity: {
          title: "Activity by Month",
        },
        recent: {
          title: "Recent Requests",
          all: "All Requests",
          category: "Category",
        },
        saved: {
          title: "Saved places",
          empty: "Saved addresses will appear here after you add them.",
          add: "Add address",
          save: "Save address",
          created: "Address saved",
          deleted: "Address deleted",
          label: "Name",
          labelPlaceholder: "For example: Home",
          type: "Place type",
          address: "Address",
          addressPlaceholder: "Street, house, entrance",
          latitude: "Latitude",
          longitude: "Longitude",
          coordinatesHint: "Coordinates are optional. If empty, we will try to find the address automatically.",
          formRequired: "Fill in the name and address.",
          addressNotFound: "Could not find the exact address. Adjust the point on the map.",
          outOfZone: "Saved addresses can only be added in Astana and within 15 km of the city. Current distance to center: {{distance}} km.",
          findOnMap: "Find on map",
          findingAddress: "Finding address...",
          resolvingAddress: "Determining address...",
          mapTitle: "Adjust the point on the map",
          mapHint: "If the address was detected incorrectly, click or drag the marker to the exact spot.",
          mapFallback: "The address was not found automatically. Select the point on the map manually.",
          approximateFound: "We found an approximate point. Check it on the map before saving.",
        },
        alerts: {
          title: "Alerts and notifications",
          empty: "No alerts or notifications yet.",
        },
      },
      settings: {
        title: "Settings",
        subtitle: "Manage your profile, password, and interface language in one place.",
        profileTitle: "Profile",
        profileDescription: "Update your personal details and profile photo.",
        editProfile: "Edit profile",
        passwordTitle: "Change password",
        passwordDescription: "Use a strong password and do not share it with anyone.",
        passwordSetupDescription: "Create a password for email or phone sign-in to your iKOMEK account.",
        currentPassword: "Enter old password",
        newPassword: "Enter new password",
        confirmPassword: "Repeat new password",
        savePassword: "Save password",
        passwordSaved: "Password updated",
        passwordFillAll: "Fill in all password fields",
        passwordMin: "New password must be at least 6 characters",
        passwordMismatch: "New passwords do not match",
        languageTitle: "Interface language",
        languageDescription: "Choose the language used across the platform.",
        saveLanguage: "Save language",
        languageSaved: "Language updated",
      },
      operator: {
        title: "Operator workspace",
        eyebrow: "Operational queue",
        description: "Prioritize the queue, review details, and update statuses without leaving the shared system.",
        update: "Update status",
        details: "Details",
        priority: "Priority",
        visibleNow: "Visible now",
        queueLabel: "Operator queue",
        searchPlaceholder: "Search by title, address, author, category, or ID",
        listTitle: "{{count}} requests in queue",
        listHint: "Cards update through search, status, category, priority, and sort filters.",
        empty: "No requests matched this operator filter.",
        department: "Department",
        departmentPlaceholder: "Responsible department",
        internalNote: "Internal note",
        internalNotePlaceholder: "Visible only to operators and admins",
        closeComment: "Closing comment",
        closeCommentPlaceholder: "Describe what was done before closing the request",
        closeCommentHelper: "Required when the status is closed.",
        closeCommentOptional: "Fill this when closing the request.",
        closeCommentRequired: "Add a closing comment before closing the request.",
        statusPriorityRequired: "Choose a priority before moving the request to work or closing it.",
        noRequestSelected: "Select a request first.",
        saveChanges: "Save changes",
        updateSuccess: "Request updated",
        stats: {
          title: "My Operator Statistics",
          totalQueue: "Total queue",
          assigned: "Taken In Work",
          closed: "Closed",
          inProgress: "Currently In Progress",
          pending: "Pending",
          queue: "Pending Queue",
          avgDays: "Average closing time",
          days: "days",
        },
        activity: {
          title: "My Monthly Workload",
        },
        recent: {
          title: "Recently Processed Requests",
        },
      },
      admin: {
        title: "Admin studio",
        description: "Monitor request metrics and publish city updates with a polished editorial flow.",
        publish: "Publish news",
        stats: {
          title: "Platform Statistics",
          totalRequests: "Total Requests",
          activeRequests: "Active requests",
          pendingRequests: "Pending requests",
          citizens: "Citizens",
          operators: "Operators",
          closed: "Closed",
          topCategory: "Top category",
        },
        activity: {
          title: "Platform Activity",
        },
        categories: {
          title: "Top Categories",
        },
        workload: {
          title: "Operator Workload",
          operator: "Operator",
          inProgress: "In Progress",
          closed: "Closed",
          total: "Total",
        },
        news: {
          created: "News item published.",
          updated: "News item updated.",
          deleted: "News item deleted.",
          fillTitleAndText: "Fill in the title and the news text.",
          selectOneType: "Select at least one news type.",
          searchPlaceholder: "Search by title or text",
          allCategories: "All categories",
          allTypes: "All types",
          periodAll: "All periods",
          periodActive: "Active",
          periodFinished: "Finished",
          periodNoPeriod: "No period",
          sortNewest: "Newest first",
          sortOldest: "Oldest first",
          editorDescription: "Create or update a news item, then refine translations if needed.",
          titleLabel: "News title",
          contentLabel: "News text",
          summaryLabel: "Summary",
          categoryLabel: "Category",
          typesLabel: "Types",
          locationLabel: "Location",
          periodStartLabel: "Period start",
          periodEndLabel: "Period end",
          translate: "Translate",
          retranslate: "Translate again",
          publishWithoutTranslation: "Publish without translation",
          localizedTitleLabel: "Localized title {{lang}}",
          localizedContentLabel: "Localized text {{lang}}",
          localizedSummaryLabel: "Localized summary {{lang}}",
          editArticleTitle: "Edit news item",
        },
      },
      emptyStates: {
        genericTitle: "Nothing to show yet",
        genericDescription: "Once the server returns records, they’ll appear here automatically.",
      },
    },
  },
  ru: {
    translation: {
      brand: { name: "iKOMEK 109", tagline: "Платформа умного города" },
      common: {
        loading: "Загрузка",
        retry: "Повторить",
        back: "Назад",
        continue: "Продолжить",
        save: "Сохранить",
        done: "Готово",
        cancel: "Отмена",
        close: "Закрыть",
        confirm: "Подтвердить",
        create: "Создать",
        update: "Обновить",
        edit: "Редактировать",
        delete: "Удалить",
        send: "Отправить",
        logout: "Выйти",
        login: "Войти",
        register: "Создать аккаунт",
        profile: "Профиль",
        dashboard: "Кабинет",
        map: "Карта",
        requests: "Заявки",
        news: "Новости",
        operator: "Операции",
        admin: "Админ",
        details: "Детали",
        chat: "Чат",
        language: "Язык",
        search: "Поиск",
        searchPlaceholder: "Поиск по заголовку, адресу или ID",
        submitRequest: "Новая заявка",
        submitRequestShort: "Заявка",
        empty: "Пока нет данных",
        public: "Публично",
        private: "Приватно",
        attachment: "Вложение",
        attachImage: "Прикрепить изображение",
        zoomIn: "Приблизить",
        zoomOut: "Отдалить",
        locateMe: "Мое местоположение",
      },
      priority: {
        unset: "Не задан",
        low: "Низкий",
        medium: "Средний",
        high: "Высокий",
      },
      nav: {
        home: "Главная",
        cabinet: "Мой кабинет",
        dashboard: "Кабинет жителя",
        requests: "Обращения",
        newRequest: "Подать проблему",
        news: "Новости и оповещения",
        map: "Карта города",
        profile: "Профиль",
        settings: "Настройки",
        operator: "Панель оператора",
        admin: "Админ-студия",
        auth: "Вход",
        sections: {
          main: "Главное меню",
          personal: "Личный кабинет",
          workplace: "Рабочее место",
          management: "Управление",
          system: "Система",
        },
      },
      shell: {
        account: "Аккаунт",
        status: "Подключение к общей платформе",
        connected: "Работает с существующей FastAPI платформой",
        guest: "Гостевой режим",
        userProfile: "Профиль пользователя",
        welcome: "Одна платформа для жителей, операторов и администрации.",
        preparing: "Готовим рабочее пространство...",
        apiError: "Ошибка API:",
        toggleNavigation: "Открыть или закрыть навигацию",
        closeNavigation: "Закрыть навигацию",
        expandSidebar: "Развернуть меню",
        collapseSidebar: "Свернуть меню",
      },
      roles: {
        admin: "Администратор",
        operator: "Оператор",
        citizen: "Житель",
        executor: "Исполнитель",
      },
      home: {
        badge: "Единая городская платформа",
        title: "Центр управления iKOMEK 109.",
        description:
          "Жители, операторы и администрация работают через общий поток заявок, единый чат и карту города.",
        primary: "Сообщить проблему",
        secondary: "Открыть карту",
        heroTrust: {
          queue: "Одна живая очередь заявок",
          roles: "Действия по ролям",
          map: "Публичная карта города",
        },
        stats: {
          requests: "Общественные заявки на карте",
          alerts: "Городские оповещения",
          inProgress: "Заявки в работе",
          categories: "Категории услуг",
        },
        overview: {
          eyebrow: "Обзор платформы",
          title: "Одна платформа для заявок, реакции и городской координации.",
          description:
            "Общее рабочее пространство для жителей, операторов и администрации.",
          boardEyebrow: "Живая нагрузка",
          boardTitle: "Что происходит в городе прямо сейчас",
          narrative:
            "Главная теперь сразу показывает рабочую картину: сколько обращений ожидают, сколько уже в обработке и сколько закрыто.",
          visibleIssues: "Живой поток городских заявок",
          pendingMetric: "Ожидают проверки",
          inProgressMetric: "В работе у оператора",
          closedMetric: "Решенные обращения",
          categoriesMetric: "Маршруты служб",
          cardOneEyebrow: "Общие данные",
          cardOneTitle: "Все городские процессы в одной системе.",
          cardOneBody:
            "Все видят одни и те же заявки, статусы и карту в единой системе.",
          cardTwoEyebrow: "Оперативные действия",
          cardTwoTitle: "Быстрый переход к решению.",
          cardTwoBody:
            "Городские команды работают через одну очередь, а не разрозненные интерфейсы.",
          cardThreeEyebrow: "Публичный просмотр",
          cardThreeTitle: "Карта города открыта для просмотра.",
          cardThreeBody:
            "Гости могут изучать публичные обращения и проблемные точки, а контакт, изменение и личные действия остаются по ролям.",
        },
        statsSection: {
          eyebrow: "Городские метрики",
          title: "Сигналы, которые оживают в момент, когда это важно.",
        },
        preview: {
          signalLabel: "Городской сигнал",
          activeMapPoints: "{{count}} активных точек на карте",
          sharedQueue:
            "Веб и мобильное приложение используют одну очередь заявок и общий городской поток.",
          categories: "Категории",
          alerts: "Оповещения",
          inProgress: "В работе",
          issueStream: "Общий поток заявок",
          disruptions: "Приоритетные уведомления и инциденты",
          inProgressDetail: "Уже приняты оператором",
          syncedCategories: "Категории услуг синхронизированы с платформой",
          serviceRoutes: "{{count}} маршрутов служб готовы",
          liveIssuePulse: "Живой пульс города",
          mapPoints: "{{count}} точек на карте",
          alertsCount: "{{count}} оповещений",
          inProgressCount: "{{count}} в работе",
          selectedIssue: "Выбранная проблема",
          publicRequest: "Публичное обращение",
          requestId: "#{{id}}",
          closeSelected: "Закрыть выбранную проблему",
          openDetails: "Открыть детали",
          signInToAct: "Войти для действий",
          authenticatedHint: "Откройте полную заявку, чтобы увидеть хронологию, чат и доступные действия роли.",
          guestHint: "Гостевой режим только для просмотра: войдите, чтобы создавать, контактировать или управлять заявками.",
        },
        newsPreview: {
          eyebrow: "Городские обновления",
          title: "Полезные городские сообщения без потери фокуса на обращениях.",
          description:
            "Ищите официальные объявления, изменения сервисов и городские публикации, пока главная остается сосредоточенной на заявках.",
          locationFallback: "Астана",
        },
        alertsStory: {
          eyebrow: "Публичная видимость",
          title: "Оповещения рядом с действием.",
          body: "Жители видят перебои и уведомления в том же интерфейсе, где создаются заявки.",
          landingMetric: "Редакционные обновления и городские оповещения на главной странице",
        },
        severity: {
          critical: "Критический",
          warning: "Предупреждение",
          information: "Информация",
          high: "Высокий",
          medium: "Средний",
          low: "Низкий",
          unknown: "Обновление",
        },
        categoriesSection: {
          eyebrow: "Какие проблемы можно сообщить",
          title: "Выберите тип городской проблемы, чтобы быстрее направить обращение в нужную службу.",
          description:
            "Каждая категория подсказывает, какие обращения можно отправить через iKOMEK 109 и что обычно решается по этому направлению.",
          modal: {
            close: "Закрыть детали категории",
            problemType: "Тип проблемы",
            detailsTitle: "Что обычно исправляется",
            examplesTitle: "Примеры",
            whenToUseTitle: "Когда выбирать эту категорию",
          },
          labels: {
            electricity: "Электричество",
            water: "Водоснабжение",
            heating: "Отопление",
            public_order: "Нарушение порядка",
            sewage: "Канализация",
            waste: "Мусор",
            roads: "Дороги",
            other: "Другое",
          },
          cards: {
            electricity: {
              description: "Проблемы с освещением, перебои электричества, аварии на линиях.",
              details:
                "Используйте эту категорию для отключений, опасных открытых кабелей, поврежденных трансформаторов и неработающего уличного освещения.",
              example: "Например: неработающие фонари, кабели, трансформаторы.",
              whenToUse:
                "Выбирайте эту категорию, если основная проблема связана с электроснабжением, светом или электрической инфраструктурой.",
            },
            water: {
              description: "Отсутствие воды, слабое давление, утечки и аварии.",
              details:
                "Используйте эту категорию, когда в доме, здании или на городской инфраструктуре есть перебои с водоснабжением, заметные протечки или нестабильное давление.",
              example: "Например: прорывы труб, перебои подачи, аварийные утечки.",
              whenToUse:
                "Выбирайте эту категорию, если проблема касается подачи воды, давления или утечки.",
            },
            heating: {
              description: "Проблемы с отоплением в домах, холодные батареи, перебои.",
              details:
                "Используйте эту категорию для холодных квартир, нестабильной подачи тепла, проблем с радиаторами и других сбоев отопительной системы.",
              example: "Например: нет тепла, неравномерный прогрев, частые отключения.",
              whenToUse:
                "Выбирайте эту категорию, если проблема связана с подачей тепла или системой отопления здания.",
            },
            public_order: {
              description: "Нарушения порядка, шум, незаконные действия в общественных местах.",
              details:
                "Используйте эту категорию для повторяющихся нарушений в общих пространствах, угроз общественной безопасности и поведения, требующего внимания города.",
              example: "Например: ночной шум, конфликты, опасное поведение.",
              whenToUse:
                "Выбирайте эту категорию, если проблема связана с безопасностью, шумом или порядком в общественных местах.",
            },
            sewage: {
              description: "Засоры канализации, неприятные запахи, аварии.",
              details:
                "Используйте эту категорию, когда канализация переполняется, стоки плохо уходят или сильный запах указывает на проблему в сети.",
              example: "Например: переполнения, засоры, запах возле дома.",
              whenToUse:
                "Выбирайте эту категорию, если проблема связана с канализацией, переливом или санитарными рисками.",
            },
            waste: {
              description: "Переполненные контейнеры, несвоевременный вывоз мусора.",
              details:
                "Используйте эту категорию для проблем с вывозом бытовых отходов, переполненных контейнеров и санитарных последствий из-за задержки уборки.",
              example: "Например: завалы, стихийные свалки, задержка уборки.",
              whenToUse:
                "Выбирайте эту категорию, если основная проблема связана с мусором, вывозом отходов или незаконным сбросом.",
            },
            roads: {
              description: "Ямы, повреждения дорог, проблемы с разметкой.",
              details:
                "Используйте эту категорию для разрушенного дорожного покрытия, опасных участков, поврежденных бордюров и отсутствующей или стертой разметки.",
              example: "Например: разбитое покрытие, бордюры, стертая разметка.",
              whenToUse:
                "Выбирайте эту категорию, если ключевая проблема связана с состоянием дороги или безопасностью движения.",
            },
            other: {
              description: "Другие городские проблемы, не попавшие в основные категории.",
              details:
                "Используйте эту категорию, если проблема требует внимания города, но ее нельзя уверенно отнести к электричеству, воде, дорогам, мусору или другой основной группе.",
              example: "Используйте, если ситуация требует внимания, но не подходит выше.",
              whenToUse:
                "Выбирайте эту категорию, если ни одна из основных категорий не описывает проблему достаточно точно.",
            },
          },
        },
        mapPreview: {
          eyebrow: "Предпросмотр карты",
          title: "Живая карта: обращения, статус и следующий шаг.",
          description:
            "Смотрите пульс города, выбирайте обращение и переходите от карты к полной записи заявки без потери контекста.",
          panelEyebrow: "Выбранный сигнал",
          panelTitle: "Контекст карты превращается в понятное действие.",
          panelDescription:
            "Предпросмотр соединяет публичную карту с операционными деталями, которые нужны жителю и оператору.",
          queueTitle: "Последние публичные обращения",
        },
        how: {
          eyebrow: "Как это работает",
          title: "От заявки до результата: что происходит на каждом этапе.",
          description:
            "Весь процесс строится вокруг одной записи заявки: на каждом этапе видно ответственного, текущий статус, передаваемые данные и результат для жителя.",
          statusLabel: "Видимый статус",
          visibleLabel: "Данные и контекст",
          resultLabel: "Результат",
          highlightOneValue: "1 ID",
          highlightOneLabel: "Одна запись заявки для веба, мобильного приложения, карты и операторских панелей.",
          highlightTwoValue: "Живой статус",
          highlightTwoLabel: "Каждый участник видит, на каком этапе находится обращение.",
          highlightThreeValue: "Действия по ролям",
          highlightThreeLabel: "Жители, операторы и администраторы работают с одними данными, но видят свои действия.",
          stepOneTitle: "Житель сообщает о проблеме",
          stepOneBody:
            "Житель отправляет заявку с категорией, локацией, фотографиями и деталями через мобильное приложение или веб.",
          stepOneMeta: "Категория, адрес, медиа и ID заявки",
          stepOneOwner: "Житель",
          stepOneStatus: "Новая заявка",
          stepOneVisible: "Категория, адрес, медиа и ID заявки",
          stepOneResult: "Заявка попадает в общую очередь и на публичную карту, если это разрешено.",
          stepTwoTitle: "Операторы обрабатывают и назначают",
          stepTwoBody:
            "Сотрудники call-центра просматривают входящие заявки, обновляют статус и координируют правильный путь реагирования.",
          stepTwoMeta: "Статус, приоритет, отдел и внутренние заметки",
          stepTwoOwner: "Оператор call-центра",
          stepTwoStatus: "На проверке / назначена",
          stepTwoVisible: "Статус, приоритет, отдел и внутренние заметки",
          stepTwoResult: "У обращения появляется ответственный маршрут, а не просто свободное сообщение.",
          stepThreeTitle: "Городские службы решают",
          stepThreeBody:
            "Заявка проходит через общие статусы с комментариями, заметками и сопроводительной коммуникацией.",
          stepThreeMeta: "Комментарии оператора и координация служб",
          stepThreeOwner: "Городская служба",
          stepThreeStatus: "В работе",
          stepThreeVisible: "Комментарии оператора, заметки службы и история координации",
          stepThreeResult: "Все видят один прогресс без повторных звонков и потерянного контекста.",
          stepFourTitle: "Житель видит результат",
          stepFourBody:
            "Тот же человек, который сообщил о проблеме, может отслеживать обновления и уверенно закрыть цикл.",
          stepFourMeta: "Публичный статус, личный кабинет и итоговое решение",
          stepFourOwner: "Житель и оператор",
          stepFourStatus: "Решена / закрыта",
          stepFourVisible: "Публичный статус, личный кабинет и итоговое решение",
          stepFourResult: "Житель понимает, что изменилось, когда это произошло и где позже найти запись.",
        },
        cta: {
          badge: "Сообщите о проблеме",
          title: "Превратите внимание в действие одним нажатием.",
          description:
            "Главная страница ведет к реальному обращению внутри единого iKOMEK-потока.",
          button: "Создать заявку",
          trust: "Единая платформа, синхронизированные статусы, один рабочий цикл.",
        },
        requestStatus: {
          pending: "Ожидает",
          in_progress: "В работе",
          closed: "Закрыта",
          open: "Открыта",
          resolved: "Решена",
          rejected: "Отклонена",
        },
        sections: {
          pulse: "Пульс города",
          pulseDescription: "Последние оповещения и новости из общего контентного потока.",
          categories: "Популярные категории",
          categoriesDescription: "Общая таксономия заявок из существующей платформы.",
          journey: "Как работает платформа",
        },
      },
      auth: {
        loginTab: "Вход",
        registerTab: "Регистрация",
        recoverTab: "Восстановление",
        login: {
          eyebrow: "Вход",
          title: "Войдите в существующий аккаунт iKOMEK",
          subtitle: "Войдите, чтобы видеть профиль, заявки и историю чатов в одном месте.",
        },
        register: {
          eyebrow: "Регистрация",
          title: "Создание аккаунта",
          subtitle: "Добро пожаловать! Пожалуйста, заполните данные для регистрации.",
        },
        recovery: {
          eyebrow: "Восстановление",
          title: "Восстановление доступа",
          subtitle: "Введите данные, чтобы восстановить доступ к вашему аккаунту.",
        },
        verification: {
          eyebrow: "Подтверждение",
          title: "Подтвердите email",
          subtitle: "Введите одноразовый код из письма, чтобы активировать аккаунт iKOMEK.",
          titleText: "Код подтверждения",
          instructions: "Введите 6-значный код, который мы отправили на ваш email.",
          sentTo: "Мы отправили код подтверждения на {{email}}.",
          code: "Код подтверждения",
          codePlaceholder: "Введите 6-значный код",
          expiresInMinutes: "Код действует {{minutes}} мин.",
          confirm: "Подтвердить email",
          resend: "Отправить код повторно",
          resendIn: "Повторно через {{seconds}}с",
          editEmail: "Изменить email",
          codeSent: "Мы отправили код подтверждения на ваш email.",
          resent: "Новый код подтверждения отправлен.",
          verified: "Email успешно подтвержден.",
          loginBlocked: "Ваш аккаунт еще не подтвержден. Введите код из письма, чтобы продолжить.",
        },
        feedback: {
          loginSuccess: "Вход выполнен",
        },
        clerk: {
          title: "Быстрый вход",
          subtitle: "Используйте Google для быстрого входа или продолжите через email или телефон iKOMEK ниже.",
          setupRequired: "Вход через Google временно недоступен. Вы можете войти через аккаунт iKOMEK ниже.",
          google: "Войти через Google",
          orLegacy: "или используйте аккаунт iKOMEK",
          signedInTitle: "Вход через Google выполнен",
          signedInHint: "Подготавливаем ваш профиль, заявки и историю чатов.",
          syncing: "Открываем ваш аккаунт...",
          localPasswordTitle: "Создайте пароль iKOMEK",
          localPasswordHint: "Задайте отдельный пароль iKOMEK, чтобы входить также через email или телефон.",
          localPassword: "Новый пароль",
          localPasswordConfirm: "Повторите пароль",
          localPasswordSave: "Сохранить пароль и продолжить",
          localPasswordSaved: "Пароль сохранен. Теперь можно входить через аккаунт iKOMEK.",
          localPasswordFillAll: "Заполните оба поля пароля",
          localPasswordMin: "Пароль должен быть не короче 6 символов",
          localPasswordMismatch: "Пароли не совпадают",
          missingToken: "Не удалось подтвердить вход. Попробуйте еще раз.",
          linkSecretRequired: "Этот email уже используется в iKOMEK. Войдите через email или телефон либо обратитесь в поддержку.",
          genericError: "Не удалось выполнить вход через Google. Попробуйте еще раз.",
        },
        recoverNotice: {
          title: "Восстановление пароля пока недоступно",
          body: "Общая серверная часть сейчас не предоставляет сценарий восстановления пароля.",
          hint: "Вы можете использовать демо-аккаунт или создать новый аккаунт на этой странице.",
        },
        fields: {
          name: "Полное имя",
          email: "Email",
          emailOrPhone: "Email или телефон",
          password: "Пароль",
          phone: "Телефон",
          language: "Предпочитаемый язык",
          required: "Обязательно",
          optional: "Необязательно",
        },
      },
      dashboard: {
        title: "Центр управления жителя",
        description: "Следите за заявками, сохраненными адресами, оповещениями и личной активностью в одном месте.",
        cards: {
          total: "Все заявки",
          active: "Активные",
          pending: "Ожидают",
          saved: "Сохраненные места",
        },
      },
      requests: {
        title: "Обращения",
        eyebrow: "Единый центр обращений",
        description: "Отслеживайте городские обращения всех пользователей и свои личные заявки в одном аккуратном рабочем пространстве.",
        heroLabel: "На экране",
        publicList: "Все пользователи",
        personalList: "Личные обращения",
        scopeLabel: "Раздел списка обращений",
        scopeAll: "Все обращения",
        scopeMine: "Мои обращения",
        statusFilterLabel: "Фильтр по статусу",
        statsTotal: "Всего обращений",
        searchPlaceholder: "Поиск по заголовку, адресу, автору или ID",
        listTitle: "{{count}} обращений",
        listHint: "Карточки разделяются по выбранному списку и обновляются через поиск, категорию, приоритет и статус.",
        requestId: "ID обращения",
        author: "Автор",
        unknownAuthor: "Автор не указан",
        moreFilters: "Фильтры",
        filterCategory: "Категория",
        allCategories: "Все категории",
        filterPriority: "Приоритет",
        allPriorities: "Все приоритеты",
        sortLabel: "Сортировка",
        sortNewest: "Сначала новые",
        sortOldest: "Сначала старые",
        sortPriority: "Сначала важные",
        paginationLabel: "Страницы обращений",
        paginationPrevious: "Назад",
        paginationNext: "Продолжить",
        filtersAll: "Все",
        filtersPending: "Ожидают",
        filtersProgress: "В работе",
        filtersClosed: "Закрыты",
        timeline: "Таймлайн статусов",
        attachments: "Вложения",
        empty: "По этому фильтру ничего не найдено.",
        statuses: {
          pending: "Ожидает",
          in_progress: "В работе",
          closed: "Закрыто",
          open: "Открыто",
          resolved: "Решено",
          rejected: "Отклонено",
        },
        priorities: {
          unset: "Не задан",
          low: "Низкий",
          medium: "Средний",
          high: "Высокий",
        },
        attachmentTypes: {
          image: "Изображение",
          document: "Документ",
        },
      },
      newRequest: {
        title: "Подать новую городскую проблему",
        description:
          "Пошагово укажите место, выберите тип проблемы, добавьте детали и отправьте обращение в удобном городском сценарии.",
        address: "Адрес",
        place: "Место",
        category: "Категория",
        reason: "Причина",
        descriptionLabel: "Описание",
        attachments: "Вложения",
        geolocate: "Использовать мою локацию",
        summary: "Сводка перед отправкой",
        locationRequired: "Укажите координаты внутри Астаны перед отправкой.",
        invalidCoordinates: "Введите корректные значения широты и долготы.",
        outOfZone: "Обращения можно создавать только в Астане и в радиусе до 15 км от города.",
        zoneHint: "Расстояние до центра Астаны: {{distance}} км. Допустимый радиус: 15 км.",
        progressLabel: "Прогресс заполнения обращения",
        optional: "Необязательно",
        success: "Обращение успешно отправлено.",
        geolocateUnavailable: "Браузер не поддерживает геолокацию.",
        geolocateError: "Не удалось определить текущее местоположение.",
        addressPlaceholder: "Улица, ориентир, двор или подъезд",
        addressSearchHint: "Начните вводить адрес в Астане. Мы попробуем автоматически поставить точку на карте.",
        addressSearchLoading: "Ищем адрес и обновляем точку на карте…",
        addressSearchNoResults: "Не удалось уверенно найти этот адрес. Укажите точку вручную на карте.",
        addressSearchError: "Поиск адреса временно недоступен. Точку всё равно можно выбрать вручную на карте.",
        addressSearchResolved: "Лучшее совпадение на карте: {{address}}",
        latitude: "Широта",
        longitude: "Долгота",
        advancedCoordinates: "Изменить координаты вручную",
        locationMapTitle: "Проверьте точку на карте",
        locationMapDescription: "Карта показывает, как система поняла адрес. Если точка неточная, перетащите маркер или кликните по карте.",
        locationMapHint: "Кликните по карте или перетащите маркер, чтобы точнее указать дом, двор или подъезд.",
        locationMapFallback: "Не удалось точно определить адрес по этой точке. Подвиньте маркер или уточните адрес вручную.",
        locationTitle: "Где произошла проблема?",
        locationDescription: "Начните с места, чтобы система сразу поняла, куда направить городскую службу.",
        typeTitle: "Что произошло?",
        typeDescription: "Сначала выберите категорию, затем причину и точное место проблемы.",
        descriptionTitle: "Опишите ситуацию",
        descriptionDescription: "Короткое и понятное описание помогает оператору быстрее приоритизировать и маршрутизировать обращение.",
        descriptionPlaceholder: "Что случилось, когда вы заметили проблему и на что стоит обратить внимание бригаде?",
        descriptionHelper: "Можно указать срочность, видимые повреждения, ориентиры или детали доступа.",
        attachmentsTitle: "Фото и настройки",
        attachmentsDescription: "Фото необязательны, но они помогают оператору оценить ситуацию до выезда.",
        uploadTitle: "Перетащите файлы сюда или загрузите с устройства",
        uploadDescription: "Лучше всего подойдут понятные фото проблемы, окружающей зоны и ориентира.",
        chooseFiles: "Выбрать файлы",
        removeFile: "Удалить",
        attachmentsCount_one: "{{count}} файл",
        attachmentsCount_few: "{{count}} файла",
        attachmentsCount_many: "{{count}} файлов",
        attachmentsCount_other: "{{count}} файла",
        visibilityTitle: "Публичность",
        visibilityDescription: "Выберите, может ли это обращение показываться в публичных списках для других жителей.",
        visibilityPublic: "Видно публично",
        visibilityPrivate: "Видно только вам и операторам",
        savedPlacesTitle: "Сохраненные места",
        savedPlacesDescription: "Выберите сохраненный адрес для быстрого старта или укажите место вручную.",
        saveCurrentLocation: "Сохранить этот адрес",
        saveCurrentLocationHint: "Добавьте выбранную точку в профиль для следующих обращений.",
        locationSaved: "Адрес сохранен в профиле",
        manualLocationTitle: "Указать вручную",
        manualLocationDescription: "Заполнить адрес и координаты самостоятельно",
        categoryTitle: "Выберите категорию",
        categoryDescription: "Укажите направление городской службы, к которому относится проблема.",
        reasonTitle: "Выберите причину",
        reasonDescription: "Причины зависят от выбранной категории.",
        reasonPlaceholderHint: "Сначала выберите категорию, чтобы увидеть подходящие причины.",
        placeTitle: "Где именно это произошло?",
        placeDescription: "Этот шаг повторяет мобильный сценарий и помогает отправить обращение в нужную службу.",
        placePlaceholder: "Например: двор, подъезд, парковка, улица",
        placeHelper: "Можно выбрать быстрый вариант или ввести более точное место вручную.",
        selectCategoryFirst: "Сначала выберите категорию, чтобы перейти к причине.",
        reviewTitle: "Проверьте перед отправкой",
        reviewDescription: "Убедитесь, что всё заполнено правильно. После отправки операторы увидят те же данные и на вебе, и в мобильном приложении.",
        reviewReady: "Все обязательные поля заполнены. Обращение готово к отправке.",
        reviewPending: "Нужно проверить еще {{count}} обязательных полей.",
        back: "Назад",
        next: "Далее",
        continueToReview: "Перейти к проверке",
        descriptionTooShort: "Описание должно содержать минимум {{count}} символов.",
        submit: "Отправить обращение",
        summaryTitle: "Живая сводка обращения",
        summaryDescription: "Эта панель обновляется по мере заполнения формы и помогает заметить пропуски до отправки.",
        summaryReadyDescription: "Все обязательные данные заполнены. Обращение можно отправлять.",
        summaryStatusReady: "Готово к отправке",
        summaryStatusDraft: "Нужно дополнить",
        summaryMapTitle: "Предпросмотр локации",
        summaryMapEmpty: "Добавьте корректные координаты, чтобы увидеть точку на карте.",
        sectionStatusReady: "Все хорошо",
        sectionStatusNeedsAttention: "Проверьте локацию",
        missingFieldsTitle: "Нужно заполнить",
        missingFields: {
          address: "Нужен адрес",
          location: "Нужна корректная точка в зоне Астаны",
          category: "Выберите категорию",
          reason: "Выберите причину",
          place: "Укажите точное место",
          description: "Добавьте краткое описание",
        },
        summaryItems: {
          address: "Адрес",
          place: "Место",
          category: "Категория",
          reason: "Причина",
          files: "Файлы",
          visibility: "Публичность",
          coordinates: "Координаты",
        },
        steps: {
          location: "Локация",
          type: "Тип",
          description: "Описание",
          attachments: "Медиа",
          review: "Проверка",
        },
        stepDescriptions: {
          location: "Подтвердите точку в Астане",
          type: "Выберите категорию, причину и место",
          description: "Добавьте контекст для оператора",
          attachments: "Фото и настройки публичности",
          review: "Проверьте готовность перед отправкой",
        },
        stepErrors: {
          location: "Добавьте адрес и выберите корректную точку в зоне Астаны.",
          type: "Перед продолжением выберите и категорию, и причину.",
          descriptionRequired: "Добавьте краткое описание перед продолжением.",
          descriptionLength: "Описание должно содержать минимум {{count}} символов.",
          review: "Перед отправкой заполните все обязательные поля.",
        },
        placeOptions: {
          apartment: "Квартира",
          house: "Дом",
          office: "Офис",
          street: "Улица",
          park: "Парк",
          other: "Другое",
        },
      },
      requestDetails: {
        overview: "Обзор",
        history: "История",
        messages: "Сообщения",
        status: "Статус",
        created: "Создано",
        category: "Категория",
        reason: "Причина",
        location: "Локация на карте",
        openChat: "Открыть чат",
        chatPreview: "Снимок чата",
        noTimeline: "История статусов пока пустая.",
        noAttachments: "Вложений пока нет.",
        noMessages: "Сообщений пока нет.",
        notFoundTitle: "Обращение не найдено",
        notFoundDescription: "Не удалось загрузить обращение с сервера.",
      },
      chat: {
        title: "Чат обращения",
        description: "Сообщения синхронизируются между жителем и оператором в реальном времени.",
        placeholder: "Введите сообщение",
        message: "Сообщение",
        sent: "Сообщение отправлено",
        requestLabel: "Обращение",
        live: "В сети",
        reconnecting: "Подключение...",
        attachment: "Вложение",
        attachMedia: "Прикрепить медиа",
        emptyTitle: "Сообщений пока нет",
        emptyDescription: "Начните диалог с оператором здесь.",
        unavailableTitle: "Чат недоступен",
        unavailableDescription: "Чатом может пользоваться только автор обращения, оператор или администратор.",
      },
      notFound: {
        title: "Страница не найдена",
        description: "Этот маршрут не входит в рабочее пространство iKOMEK.",
      },
      aiAssistant: {
        label: "AI ассистент",
        title: "AI ассистент",
        subtitle: "Помогает с заявками, статусами и навигацией",
        greeting: "Здравствуйте! Я помогу разобраться с iKOMEK 109, заявками, новостями и статусами.",
        placeholder: "Напишите вопрос...",
        empty: "Спросите, как создать заявку или где посмотреть статус.",
        configured: "Gemini API key не настроен",
        close: "Закрыть AI ассистент",
        failed: "Не удалось получить ответ. Проверьте backend и настройки AI.",
      },
      news: {
        title: "Городские новости и оповещения",
        description: "Аккуратный редакционный интерфейс для существующего городского контент-потока.",
        searchPlaceholder: "Поиск по заголовку или тексту",
        allCategories: "Все категории",
        allTypes: "Все типы",
        periodAll: "Все периоды",
        periodActive: "Активные",
        periodFinished: "Завершённые",
        periodNoPeriod: "Без периода",
        sortNewest: "Сначала новые",
        sortOldest: "Сначала старые",
        noResults: "Ничего не найдено",
        emptyFiltered: "Измените фильтры или поисковый запрос.",
        modalTitle: "Новость",
        createdAgo: "Создано {{count}} д назад",
        createdHoursAgo: "Создано {{count}} ч назад",
        createdJustNow: "Только что создано",
        period: "ПЕРИОД",
        location: "ЛОКАЦИЯ",
        types: {
          emergency: "Аварийные работы",
          weather: "Погодные условия",
          planned: "Плановые работы",
          road: "Дорожные ситуации",
          education: "Управление образования",
          events: "Мероприятия города",
        },
        categories: {
          roads: "Дороги",
          utilities: "Коммунальные услуги",
          transport: "Транспорт",
          education: "Образование",
          weather: "Погода",
          improvement: "Благоустройство",
        },
        translationStatus: {
          translated: "Переведено",
          skipped: "Без перевода",
          failed: "Ошибка перевода",
        },
      },
      categories: {
        electricity: "Электричество",
        water: "Водоснабжение",
        heating: "Отопление",
        publicOrder: "Нарушение порядка",
        sewage: "Канализация",
        waste: "Мусор",
        roads: "Дороги",
        streetLighting: "Уличное освещение",
        other: "Другое",
      },
      placeTypes: {
        apartment: "Квартира",
        house: "Частный дом",
        office: "Офис",
        street: "Улица",
        park: "Парк/сквер",
        entrance: "Подъезд",
        yard: "Двор",
        parking: "Паркинг",
        other: "Другое",
      },
      problemTypes: {
        electricity: {
          power_outage: "Нет электричества",
          voltage_issue: "Перепады напряжения",
          damaged_cables: "Поврежденные кабели",
          street_light: "Не работает уличный фонарь",
          sparks: "Искры или риск возгорания",
          other: "Другая проблема с электричеством",
        },
        water: {
          no_water: "Нет водоснабжения",
          low_pressure: "Низкое давление воды",
          pipe_leak: "Протечка трубы",
          dirty_water: "Грязная или мутная вода",
          sewage: "Проблема с канализацией",
          other: "Другая проблема с водой",
        },
        sewage: {
          blockage: "Засор канализации",
          leak: "Утечка канализации",
          odor: "Неприятный запах канализации",
          overflow: "Переполнение канализации",
          manhole: "Открытый или поврежденный люк",
          other: "Другая проблема с канализацией",
        },
        roads: {
          pothole: "Яма на дороге",
          damaged_pavement: "Поврежденное покрытие",
          road_sign: "Нет или поврежден дорожный знак",
          traffic_light: "Проблема со светофором",
          road_marking: "Нужна дорожная разметка",
          other: "Другая проблема с дорогой",
        },
        public_order: {
          noise: "Жалоба на шум",
          illegal_parking: "Неправильная парковка",
          vandalism: "Вандализм",
          abandoned_vehicle: "Брошенный автомобиль",
          stray_animals: "Бродячие животные",
          other: "Другая проблема с порядком",
        },
        waste: {
          overflowing: "Переполненный мусорный контейнер",
          illegal_dump: "Несанкционированная свалка",
          missed_collection: "Мусор не вывезли",
          hazardous: "Опасные отходы",
          bulk_waste: "Нужно вывезти крупногабаритный мусор",
          other: "Другая проблема с мусором",
        },
        heating: {
          no_heating: "Нет отопления",
          radiator_leak: "Протечка радиатора",
          cold_apartment: "Холодно в квартире",
          overheating: "Слишком жарко",
          noise: "Шум в системе отопления",
          other: "Другая проблема с отоплением",
        },
        street_lighting: {
          lamp_out: "Фонарь не работает",
          flickering: "Фонарь мигает",
          damaged_pole: "Поврежден столб",
          dark_area: "Участок плохо освещен",
          timer: "Сбой таймера",
          other: "Другая проблема с освещением",
        },
        other: {
          general: "Общая жалоба",
          suggestion: "Предложение",
          question: "Вопрос",
          other: "Другое",
        },
      },
      reasons: {
        electricity: {
          infrastructure: "Неисправность инфраструктуры",
          weather: "Повреждение из-за погоды",
          overload: "Перегрузка системы",
          maintenance: "Требуется обслуживание",
          accident: "Авария или внешнее повреждение",
          unknown: "Причина неизвестна",
        },
        water: {
          pipe_burst: "Прорыв трубы",
          maintenance: "Плановые работы",
          infrastructure: "Изношенная инфраструктура",
          external: "Внешнее повреждение",
          pressure: "Проблема с давлением",
          unknown: "Причина неизвестна",
        },
        sewage: {
          pipe: "Проблема с трубой",
          blockage: "Засор",
          wear: "Износ трубы",
          overflow: "Переполнение",
          odor: "Проблема с запахом",
          infrastructure: "Изношенная инфраструктура",
          rain: "Сильный дождь",
          maintenance: "Требуется обслуживание",
          unknown: "Причина неизвестна",
        },
        roads: {
          weather: "Износ из-за погоды",
          traffic: "Повреждение от транспорта",
          construction: "Повреждение из-за строительства",
          age: "Износ со временем",
          accident: "Повреждение после аварии",
          unknown: "Причина неизвестна",
        },
        public_order: {
          resident: "Жалоба жителей",
          safety: "Угроза общественной безопасности",
          community: "Проблема для района",
          legal: "Нарушение закона",
          recurring: "Повторяющаяся проблема",
          other: "Другая причина",
        },
        waste: {
          schedule: "Проблема графика",
          container: "Повреждение контейнера",
          illegal: "Незаконный выброс",
          volume: "Увеличение объема",
          access: "Проблема доступа",
          other: "Другая причина",
        },
        heating: {
          boiler: "Проблема с котлом",
          pipe: "Проблема с трубой",
          system: "Сбой системы",
          regulation: "Регулировка температуры",
          maintenance: "Требуется обслуживание",
          unknown: "Причина неизвестна",
        },
        street_lighting: {
          bulb: "Перегоревшая лампа",
          electrical: "Электрическая неисправность",
          vandalism: "Вандализм",
          timer: "Сбой таймера",
          age: "Износ со временем",
          unknown: "Причина неизвестна",
        },
        other: {
          quality: "Качество услуги",
          safety: "Проблема безопасности",
          environment: "Экологическая проблема",
          other: "Другое",
        },
      },
      map: {
        title: "Живая карта города",
        subtitle: "Астана · заявок в реальном времени",
        subtitleWithCount: "Астана · {{count}} заявок в реальном времени",
        description: "Исследуйте кластеризованные публичные заявки, тепловые зоны и активность районов.",
        all: "Все заявки",
        mine: "Мои заявки",
        heatmap: "Тепловая карта",
        filters: {
          allRequests: "Все заявки",
          myRequests: "Мои заявки",
          heatmap: "Тепловая карта",
          allCategories: "Все категории",
          allStatuses: "Все статусы",
          allPriorities: "Все приоритеты",
          today: "Сегодня",
          week: "Неделя",
          month: "Месяц",
          dateRangeHelp: "Выберите до 7 дней подряд",
          previousMonth: "Предыдущий месяц",
          nextMonth: "Следующий месяц",
          reset: "Сбросить фильтры",
          showFilters: "Показать фильтры",
          hideFilters: "Скрыть фильтры",
        },
        analytics: {
          title: "Аналитика города",
          density: "Плотность по районам",
          categories: "Популярные категории",
          timeline: "Динамика по месяцам",
          timelineMonths: "Динамика по месяцам",
          timelineWeekdays: "Динамика по дням недели",
          timelineHours: "Динамика по часам",
          timelineView: "Вид графика",
          timelineSelectedPeriod: "Период",
          timelineAllMonths: "Все месяцы",
          chartTotal: "Всего",
          hotspots: "Топ проблемных мест",
          activity: "Активность по дням и часам",
          otherDistrict: "Другое",
          seriesAll: "Все заявки",
          weekdays: {
            mon: "Пн",
            tue: "Вт",
            wed: "Ср",
            thu: "Чт",
            fri: "Пт",
            sat: "Сб",
            sun: "Вс",
          },
          requestsCount: "{{count}} заявок",
          insufficientData: "Недостаточно данных для аналитики",
        },
      },
      profile: {
        title: "Профиль и сохраненные адреса",
        addLocation: "Добавить место",
        notifications: "Уведомления",
      },
      savedLocationTypes: {
        home: "Дом",
        work: "Работа",
        study: "Учеба",
        family: "Семья",
        other: "Другое",
      },
      cabinet: {
        title: "Личный кабинет",
        memberSince: "Житель с",
        editProfile: "Редактировать профиль",
        logout: "Выйти",
        comingSoon: "Скоро будет доступно",
        profileEditor: {
          title: "Ваши данные",
          displayName: "Как к вам обращаться?",
          displayNamePlaceholder: "Имя для обращений",
          displayNameHint: "Это имя отображается в ваших отзывах и комментариях.",
          changePhoto: "Изменить фото",
          removePhoto: "Удалить фото",
          cropPhoto: "Обрезать фото",
          photoScale: "Размер фото",
          photoError: "Не удалось подготовить фото",
          personalData: "Персональные данные",
          firstName: "Имя",
          lastName: "Фамилия",
          gender: "Пол",
          birthDate: "Дата рождения",
          phone: "Телефон",
          language: "Язык интерфейса",
          notifications: "Получать городские уведомления",
          save: "Сохранить",
          saved: "Профиль обновлен",
          nameRequired: "Введите имя и фамилию",
          nameLettersOnly: "В имени и фамилии используйте только буквы, дефис можно ставить только между частями имени",
          phoneInvalid: "Введите казахстанский номер: 11 цифр, начиная с 7",
          birthDateInvalid: "Введите корректную дату рождения, не из будущего",
          genders: {
            male: "М",
            female: "Ж",
          },
        },
        stats: {
          title: "Моя статистика",
          total: "Всего заявок",
          closed: "Закрыто",
          inProgress: "В работе",
          pending: "Ожидает",
          avgDays: "Среднее время закрытия",
          days: "дней",
        },
        activity: {
          title: "Активность по месяцам",
        },
        recent: {
          title: "Последние заявки",
          all: "Все заявки",
          category: "Категория",
        },
        saved: {
          title: "Сохраненные места",
          empty: "Сохраненные адреса появятся здесь после добавления.",
          add: "Добавить адрес",
          save: "Сохранить адрес",
          created: "Адрес сохранен",
          deleted: "Адрес удален",
          label: "Название",
          labelPlaceholder: "Например: Дом",
          type: "Тип места",
          address: "Адрес",
          addressPlaceholder: "Улица, дом, подъезд",
          latitude: "Широта",
          longitude: "Долгота",
          coordinatesHint: "Координаты необязательны. Если оставить пустыми, мы попробуем найти адрес автоматически.",
          formRequired: "Заполните название и адрес.",
          addressNotFound: "Не удалось найти точный адрес. Уточните точку на карте.",
          outOfZone: "Сохраненные адреса можно добавлять только в Астане и в радиусе до 15 км от города. Сейчас до центра: {{distance}} км.",
          findOnMap: "Найти на карте",
          findingAddress: "Ищем адрес...",
          resolvingAddress: "Определяем адрес...",
          mapTitle: "Уточните точку на карте",
          mapHint: "Если адрес определился неточно, кликните по карте или перетащите маркер в нужное место.",
          mapFallback: "Адрес не найден автоматически. Выберите точку на карте вручную.",
          approximateFound: "Мы нашли примерную точку. Проверьте ее на карте перед сохранением.",
        },
        alerts: {
          title: "Оповещения и уведомления",
          empty: "Пока нет оповещений или уведомлений.",
        },
      },
      settings: {
        title: "Настройки",
        subtitle: "Управляйте профилем, паролем и языком интерфейса в одном месте.",
        profileTitle: "Профиль",
        profileDescription: "Измените личные данные и фотографию профиля.",
        editProfile: "Редактировать профиль",
        passwordTitle: "Смена пароля",
        passwordDescription: "Используйте надежный пароль и никому его не сообщайте.",
        passwordSetupDescription: "Создайте пароль для входа в аккаунт iKOMEK через email или телефон.",
        currentPassword: "Введите старый пароль",
        newPassword: "Введите новый пароль",
        confirmPassword: "Повторите новый пароль",
        savePassword: "Сохранить пароль",
        passwordSaved: "Пароль обновлен",
        passwordFillAll: "Заполните все поля пароля",
        passwordMin: "Новый пароль должен быть не короче 6 символов",
        passwordMismatch: "Новые пароли не совпадают",
        languageTitle: "Язык интерфейса",
        languageDescription: "Выберите язык, который будет использоваться на платформе.",
        saveLanguage: "Сохранить язык",
        languageSaved: "Язык обновлен",
      },
      operator: {
        title: "Рабочее место оператора",
        eyebrow: "Операционная очередь",
        description: "Приоритизируйте очередь, проверяйте детали и обновляйте статусы внутри общей системы.",
        update: "Обновить статус",
        details: "Детали",
        priority: "Приоритет",
        visibleNow: "На экране",
        queueLabel: "Очередь оператора",
        searchPlaceholder: "Поиск по заголовку, адресу, автору, категории или ID",
        listTitle: "{{count}} обращений в очереди",
        listHint: "Карточки обновляются через поиск, статус, категорию, приоритет и сортировку.",
        empty: "По этому операторскому фильтру ничего не найдено.",
        department: "Отдел",
        departmentPlaceholder: "Ответственный отдел",
        internalNote: "Внутренняя заметка",
        internalNotePlaceholder: "Видно только операторам и администраторам",
        closeComment: "Комментарий к закрытию",
        closeCommentPlaceholder: "Опишите, что сделано перед закрытием обращения",
        closeCommentHelper: "Обязательно при статусе «Закрыто».",
        closeCommentOptional: "Заполните при закрытии обращения.",
        closeCommentRequired: "Добавьте комментарий перед закрытием обращения.",
        statusPriorityRequired: "Выберите приоритет перед переводом обращения в работу или закрытием.",
        noRequestSelected: "Сначала выберите обращение.",
        saveChanges: "Сохранить изменения",
        updateSuccess: "Обращение обновлено",
        stats: {
          title: "Моя статистика оператора",
          totalQueue: "Всего в очереди",
          assigned: "Взято в работу",
          closed: "Закрыто",
          inProgress: "Сейчас в работе",
          pending: "Ожидают",
          queue: "Очередь ожидает",
          avgDays: "Среднее время закрытия",
          days: "дней",
        },
        activity: {
          title: "Моя нагрузка по месяцам",
        },
        recent: {
          title: "Последние обработанные заявки",
        },
      },
      admin: {
        title: "Админ-студия",
        description: "Контролируйте метрики и публикуйте городские обновления в аккуратном рабочем процессе.",
        publish: "Опубликовать новость",
        stats: {
          title: "Статистика платформы",
          totalRequests: "Всего заявок",
          activeRequests: "Активные заявки",
          pendingRequests: "Ожидающие заявки",
          citizens: "Жителей",
          operators: "Операторов",
          closed: "Закрыто",
          topCategory: "Топ-категория",
        },
        activity: {
          title: "Активность платформы",
        },
        categories: {
          title: "Топ категорий",
        },
        workload: {
          title: "Нагрузка операторов",
          operator: "Оператор",
          inProgress: "В работе",
          closed: "Закрыто",
          total: "Всего",
        },
        news: {
          created: "Новость опубликована.",
          updated: "Новость обновлена.",
          deleted: "Новость удалена.",
          fillTitleAndText: "Заполните заголовок и текст новости.",
          selectOneType: "Выберите хотя бы один тип новости.",
          searchPlaceholder: "Поиск по заголовку или тексту",
          allCategories: "Все категории",
          allTypes: "Все типы",
          periodAll: "Все периоды",
          periodActive: "Активные",
          periodFinished: "Завершённые",
          periodNoPeriod: "Без периода",
          sortNewest: "Сначала новые",
          sortOldest: "Сначала старые",
          editorDescription: "Создайте или обновите новость, затем при необходимости отредактируйте переводы.",
          titleLabel: "Заголовок новости",
          contentLabel: "Текст новости",
          summaryLabel: "Краткое описание",
          categoryLabel: "Категория",
          typesLabel: "Типы",
          locationLabel: "Локация",
          periodStartLabel: "Начало периода",
          periodEndLabel: "Окончание периода",
          translate: "Перевести",
          retranslate: "Перевести заново",
          publishWithoutTranslation: "Опубликовать без перевода",
          localizedTitleLabel: "Локализованный заголовок {{lang}}",
          localizedContentLabel: "Локализованный текст {{lang}}",
          localizedSummaryLabel: "Локализованное краткое описание {{lang}}",
          editArticleTitle: "Редактировать новость",
        },
      },
      emptyStates: {
        genericTitle: "Пока нечего показывать",
        genericDescription: "Как только сервер вернет записи, они автоматически появятся здесь.",
      },
    },
  },
  kz: {
    translation: {
      brand: { name: "iKOMEK 109", tagline: "Ақылды қала платформасы" },
      common: {
        loading: "Жүктелуде",
        retry: "Қайталау",
        back: "Артқа",
        continue: "Жалғастыру",
        save: "Сақтау",
        done: "Дайын",
        cancel: "Бас тарту",
        close: "Жабу",
        confirm: "Растау",
        create: "Құру",
        update: "Жаңарту",
        edit: "Өңдеу",
        delete: "Жою",
        send: "Жіберу",
        logout: "Шығу",
        login: "Кіру",
        register: "Тіркелу",
        profile: "Профиль",
        dashboard: "Кабинет",
        map: "Карта",
        requests: "Өтінімдер",
        news: "Жаңалықтар",
        operator: "Операциялар",
        admin: "Әкімші",
        details: "Деректер",
        chat: "Чат",
        language: "Тіл",
        search: "Іздеу",
        searchPlaceholder: "Тақырып, мекенжай немесе ID бойынша іздеу",
        submitRequest: "Жаңа өтінім",
        submitRequestShort: "Өтінім",
        empty: "Әзірге дерек жоқ",
        public: "Ашық",
        private: "Жеке",
        attachment: "Тіркеме",
        attachImage: "Сурет тіркеу",
        zoomIn: "Жақындату",
        zoomOut: "Алыстату",
        locateMe: "Менің орным",
      },
      priority: {
        unset: "Белгіленбеген",
        low: "Төмен",
        medium: "Орташа",
        high: "Жоғары",
      },
      nav: {
        home: "Басты бет",
        cabinet: "Менің кабинетім",
        dashboard: "Тұрғын кабинеті",
        requests: "Өтініштер",
        newRequest: "Мәселе жіберу",
        news: "Жаңалықтар мен ескертулер",
        map: "Қала картасы",
        profile: "Профиль",
        settings: "Баптаулар",
        operator: "Оператор панелі",
        admin: "Әкімші студиясы",
        auth: "Кіру",
        sections: {
          main: "Басты мәзір",
          personal: "Жеке кабинет",
          workplace: "Жұмыс орны",
          management: "Басқару",
          system: "Жүйе",
        },
      },
      shell: {
        account: "Аккаунт",
        status: "Ортақ платформа байланысы",
        connected: "Қолданыстағы FastAPI платформасымен жұмыс істейді",
        guest: "Қонақ режимі",
        userProfile: "Пайдаланушы профилі",
        welcome: "Тұрғындарға, операторларға және әкімдікке арналған бір платформа.",
        preparing: "Жұмыс кеңістігі дайындалуда...",
        apiError: "API қатесі:",
        toggleNavigation: "Навигацияны ашу немесе жабу",
        closeNavigation: "Навигацияны жабу",
        expandSidebar: "Мәзірді кеңейту",
        collapseSidebar: "Мәзірді жинау",
      },
      roles: {
        admin: "Әкімші",
        operator: "Оператор",
        citizen: "Тұрғын",
        executor: "Орындаушы",
      },
      home: {
        badge: "Бірыңғай қалалық платформа",
        title: "iKOMEK 109 басқару орталығы.",
        description:
          "Тұрғындар, операторлар және әкімдік ортақ өтінімдер ағымы, бірыңғай чат және қала картасы арқылы жұмыс істейді.",
        primary: "Мәселе жөнінде хабарлау",
        secondary: "Картаны ашу",
        heroTrust: {
          queue: "Бір тірі өтінім кезегі",
          roles: "Рөлге байланысты әрекеттер",
          map: "Қаланың ашық картасы",
        },
        stats: {
          requests: "Картадағы ашық мәселелер",
          alerts: "Белсенді ескертулер",
          inProgress: "Жұмыстағы өтінімдер",
          categories: "Қызмет санаттары",
        },
        overview: {
          eyebrow: "Платформаға шолу",
          title: "Өтініштер, әрекет және қала үйлестіруі үшін бір платформа.",
          description:
            "Тұрғындар, операторлар және әкімшілік үшін ортақ жұмыс кеңістігі.",
          boardEyebrow: "Жанды жүктеме",
          boardTitle: "Қалада қазір не болып жатыр",
          narrative:
            "Басты бет жұмыс көрінісін бірден көрсетеді: қанша өтінім күтуде, қаншасы өңделуде және қаншасы жабылды.",
          visibleIssues: "Қалалық өтінімдердің шынайы ағыны",
          pendingMetric: "Тексеруді күтуде",
          inProgressMetric: "Оператор жұмысында",
          closedMetric: "Шешілген өтінімдер",
          categoriesMetric: "Қызмет бағыттары",
          cardOneEyebrow: "Ортақ дерек",
          cardOneTitle: "Барлық қала процестері бір жүйеде.",
          cardOneBody:
            "Барлығы бірдей өтінімдерді, статустарды және карта контентін бір жүйеде көреді.",
          cardTwoEyebrow: "Жылдам әрекет",
          cardTwoTitle: "Хабарламадан шешімге дейін жылдам орындалу.",
          cardTwoBody:
            "Қала командалары жеке беттер емес, ортақ кезек арқылы ынтымақтасады.",
          cardThreeEyebrow: "Қоғамдық қарау",
          cardThreeTitle: "Қала картасы қарауға ашық.",
          cardThreeBody:
            "Қонақтар ашық өтінімдер мен проблемалық нүктелерді қарай алады, ал байланысу, өзгерту және жеке әрекеттер рөлдерге байланысты қалады.",
        },
        statsSection: {
          eyebrow: "Қала метрикалары",
          title: "Маңызды болған сәтте тірі көрінетін сигналдар.",
        },
        preview: {
          signalLabel: "Ортақ қала сигналы",
          activeMapPoints: "Картадағы белсенді нүктелер: {{count}}",
          sharedQueue:
            "Веб пен мобильді қосымша бір өтінім кезегін және бір қалалық контент ағынын пайдаланады.",
          categories: "Санаттар",
          alerts: "Ескертулер",
          inProgress: "Жұмыста",
          issueStream: "Қалалық өтінімдердің ортақ ағыны",
          disruptions: "Басым хабарламалар мен үзілістер",
          inProgressDetail: "Операторлар қабылдаған",
          syncedCategories: "Қызмет санаттары платформамен синхрондалған",
          serviceRoutes: "{{count}} қызмет бағыты дайын",
          liveIssuePulse: "Қалалық өтінімдер пульсі",
          mapPoints: "Карта нүктелері: {{count}}",
          alertsCount: "Ескертулер: {{count}}",
          inProgressCount: "Жұмыста: {{count}}",
          selectedIssue: "Таңдалған мәселе",
          publicRequest: "Ашық өтінім",
          requestId: "#{{id}}",
          closeSelected: "Таңдалған мәселені жабу",
          openDetails: "Мәліметті ашу",
          signInToAct: "Әрекет үшін кіру",
          authenticatedHint: "Толық өтінімді ашып, хронологияны, чатты және рөлге қолжетімді әрекеттерді көріңіз.",
          guestHint: "Қонақ режимі тек қарауға арналған: өтінім жасау, байланысу немесе басқару үшін кіріңіз.",
        },
        newsPreview: {
          eyebrow: "Қалалық жаңартулар",
          title: "Өтінімдер ағымынан алыстамай пайдалы қалалық хабарламаларды көріңіз.",
          description:
            "Ресми хабарландыруларды, қызмет өзгерістерін және қалалық жарияланымдарды іздеңіз, ал басты бет өтінімдерге бағытталған күйде қалады.",
          locationFallback: "Астана",
        },
        alertsStory: {
          eyebrow: "Қоғамдық көріну",
          title: "Ескертулер әрекетке жақын көрінеді.",
          body: "Тұрғындар үзілістерді және хабарландыруларды сол интерфейстен көре алады, онда өтінімдер жасалады.",
          landingMetric: "Басты беттегі редакциялық жаңартулар мен қалалық ескертулер",
        },
        severity: {
          critical: "Сындарлы",
          warning: "Ескерту",
          information: "Ақпарат",
          high: "Жоғары",
          medium: "Орташа",
          low: "Төмен",
          unknown: "Жаңарту",
        },
        categoriesSection: {
          eyebrow: "Қандай мәселелерді хабарлауға болады",
          title: "Қай қызметке тезірек бағыттау үшін қалалық мәселе түрін таңдаңыз.",
          description:
            "Әр санат iKOMEK 109 арқылы қандай өтінім жіберуге болатынын және әдетте қандай мәселе шешілетінін түсіндіреді.",
          modal: {
            close: "Санат мәліметін жабу",
            problemType: "Мәселе түрі",
            detailsTitle: "Әдетте не жөнделеді",
            examplesTitle: "Мысалдар",
            whenToUseTitle: "Бұл санатты қашан таңдау керек",
          },
          labels: {
            electricity: "Электр қуаты",
            water: "Сумен қамтамасыз ету",
            heating: "Жылыту",
            public_order: "Тәртіп бұзушылық",
            sewage: "Кәріз",
            waste: "Қоқыс",
            roads: "Жолдар",
            other: "Басқа",
          },
          cards: {
            electricity: {
              description: "Жарықтың өшуі, электр үзілістері, желідегі апаттар.",
              details:
                "Бұл санатты өшірулер, ашық қауіпті кабельдер, зақымдалған трансформаторлар және істемейтін көше жарығы үшін пайдаланыңыз.",
              example: "Мысалы: шамдар істемейді, кабель зақымы, трансформатор ақауы.",
              whenToUse:
                "Негізгі мәселе электр, жарық немесе электр инфрақұрылымына қатысты болса, осы санатты таңдаңыз.",
            },
            water: {
              description: "Судың болмауы, қысымның әлсіздігі, ағулар мен апаттар.",
              details:
                "Үйде, ғимаратта немесе сыртқы инфрақұрылымда су беру үзіліп, айқын ағу не қысым тұрақсыз болса, осы санатты пайдаланыңыз.",
              example: "Мысалы: құбыр жарылуы, су беру үзілуі, апаттық ағулар.",
              whenToUse:
                "Негізгі мәселе су беру, қысым немесе ағуға қатысты болса, осы санатты таңдаңыз.",
            },
            heating: {
              description: "Үйлердегі жылыту мәселелері, суық батареялар, үзілістер.",
              details:
                "Бұл санатты пәтердің суық болуы, жылудың тұрақсыз берілуі, радиатор мәселелері және басқа жылыту жүйесі ақаулары үшін пайдаланыңыз.",
              example: "Мысалы: жылу жоқ, біркелкі жылымайды, жиі өшеді.",
              whenToUse:
                "Мәселе ғимараттың жылу беруіне немесе жылыту жүйесіне қатысты болса, осы санатты таңдаңыз.",
            },
            public_order: {
              description: "Тәртіп бұзушылық, шу, қоғамдық орындардағы заңсыз әрекеттер.",
              details:
                "Бұл санатты ортақ кеңістіктегі қайталанатын бұзушылықтар, қоғамдық қауіпсіздікке қатер және қала назарын қажет ететін әрекеттер үшін пайдаланыңыз.",
              example: "Мысалы: түнгі шу, жанжал, қауіпті мінез-құлық.",
              whenToUse:
                "Мәселе қауіпсіздікке, шуға немесе қоғамдық орындардағы тәртіпке қатысты болса, осы санатты таңдаңыз.",
            },
            sewage: {
              description: "Кәріз бітелуі, жағымсыз иіс, апаттық жағдайлар.",
              details:
                "Кәріз тасып, су кетпей қалып немесе қатты иіс желідегі ақауды көрсетсе, осы санатты пайдаланыңыз.",
              example: "Мысалы: тасу, бітелу, үй маңындағы өткір иіс.",
              whenToUse:
                "Мәселе кәрізге, тасуға немесе санитарлық қауіпке қатысты болса, осы санатты таңдаңыз.",
            },
            waste: {
              description: "Толып кеткен контейнерлер, қоқысты уақытында шығармау.",
              details:
                "Бұл санатты тұрмыстық қалдықтарды шығарудағы қиындықтар, толған контейнерлер және тазалау кешігуінен туған санитарлық мәселелер үшін пайдаланыңыз.",
              example: "Мысалы: жиналып қалған қоқыс, заңсыз төгу, тазалау кешігуі.",
              whenToUse:
                "Негізгі мәселе қоқыс жинау, контейнерлердің толуы немесе заңсыз төгу болса, осы санатты таңдаңыз.",
            },
            roads: {
              description: "Шұңқырлар, жол зақымдары, таңбалау мәселелері.",
              details:
                "Бұл санатты бұзылған жол жабыны, қауіпті учаскелер, бүлінген бордюрлер және жоқ не өшкен таңбалау үшін пайдаланыңыз.",
              example: "Мысалы: бұзылған жол жабыны, бордюр, өшкен сызықтар.",
              whenToUse:
                "Негізгі мәселе жолдың жағдайына немесе қозғалыс қауіпсіздігіне қатысты болса, осы санатты таңдаңыз.",
            },
            other: {
              description: "Негізгі санаттарға кірмейтін басқа қалалық мәселелер.",
              details:
                "Егер мәселе қала назарын қажет етсе, бірақ оны электр, су, жол, қоқыс не басқа негізгі топқа сенімді түрде жатқызу мүмкін болмаса, осы санатты пайдаланыңыз.",
              example: "Егер мәселе назар талап етсе, бірақ жоғарыдағы санаттарға сай келмесе.",
              whenToUse:
                "Егер негізгі санаттардың ешқайсысы мәселеңізді дәл сипаттамаса, осы санатты таңдаңыз.",
            },
          },
        },
        mapPreview: {
          eyebrow: "Карта алдын ала қарауы",
          title: "Тірі карта алдын ала қарауы: өтінімдер, статус және келесі қадам.",
          description:
            "Қала пульсін көріңіз, өтінімді таңдаңыз және карта контекстінен толық жазбаға тез өтіңіз.",
          panelEyebrow: "Таңдалған сигнал",
          panelTitle: "Карта контексті түсінікті әрекетке айналады.",
          panelDescription:
            "Алдын ала қарау ашық картаны тұрғын мен операторға қажет операциялық мәліметтермен байланыстырады.",
          queueTitle: "Соңғы ашық өтінімдер",
        },
        how: {
          eyebrow: "Қалай жұмыс істейді",
          title: "Өтінімнен нәтижеге дейін: әр кезеңде не болады.",
          description:
            "Процесс бір өтінім жазбасының айналасында құрылады: әр кезеңде жауапты адам, көрінетін статус, алға өтетін деректер және тұрғын алатын нәтиже көрсетіледі.",
          statusLabel: "Көрінетін статус",
          visibleLabel: "Деректер мен контекст",
          resultLabel: "Нәтиже",
          highlightOneValue: "1 ID",
          highlightOneLabel: "Веб, мобильді қосымша, карта және оператор панельдері үшін бір өтінім жазбасы.",
          highlightTwoValue: "Тірі статус",
          highlightTwoLabel: "Әр қатысушы өтінім өмірлік циклдің қай кезеңінде екенін көреді.",
          highlightThreeValue: "Рөлдік әрекеттер",
          highlightThreeLabel: "Тұрғындар, операторлар және әкімшілер бір дерекпен жұмыс істеп, өз әрекеттерін көреді.",
          stepOneTitle: "Тұрғын мәселені хабарлайды",
          stepOneBody:
            "Тұрғын мобильді қосымша не веб арқылы санат, орын, фото және толық мәліметпен өтінім жібереді.",
          stepOneMeta: "Санат, мекенжай, медиа және өтінім ID",
          stepOneOwner: "Тұрғын",
          stepOneStatus: "Жаңа өтінім",
          stepOneVisible: "Санат, мекенжай, медиа және өтінім ID",
          stepOneResult: "Өтінім ортақ кезекке және рұқсат етілсе ашық картаға түседі.",
          stepTwoTitle: "Операторлар сұрыптап, тағайындайды",
          stepTwoBody:
            "Call-орталық қызметкерлері кіріс өтінімдерді қарап, статусын жаңартып, дұрыс әрекет жолын үйлестіреді.",
          stepTwoMeta: "Статус, басымдық, бөлім және ішкі жазбалар",
          stepTwoOwner: "Call-орталық операторы",
          stepTwoStatus: "Тексеруде / тағайындалды",
          stepTwoVisible: "Статус, басымдық, бөлім және ішкі жазбалар",
          stepTwoResult: "Өтінім бос хабарлама емес, жауапты бағытқа түседі.",
          stepThreeTitle: "Қалалық қызметтер шешеді",
          stepThreeBody:
            "Өтінім ортақ статустар, пікірлер, ескертпелер және сүйемелдеуші байланыс арқылы алға жылжиды.",
          stepThreeMeta: "Оператор пікірлері және қызметтерді үйлестіру",
          stepThreeOwner: "Қалалық қызмет",
          stepThreeStatus: "Орындалуда",
          stepThreeVisible: "Оператор пікірлері, қызмет жазбалары және үйлестіру тарихы",
          stepThreeResult: "Барлығы бір прогресті көреді, қайталама қоңыраулар мен жоғалған контекст азаяды.",
          stepFourTitle: "Тұрғын нәтижені көреді",
          stepFourBody:
            "Мәселені хабарлаған сол адам жаңартуларды бақылап, циклді сенімді түрде жаба алады.",
          stepFourMeta: "Ашық статус, жеке кабинет және соңғы шешім",
          stepFourOwner: "Тұрғын және оператор",
          stepFourStatus: "Шешілді / жабылды",
          stepFourVisible: "Ашық статус, жеке кабинет және соңғы шешім",
          stepFourResult: "Тұрғын не өзгергенін, қашан өзгергенін және жазбаны кейін қайдан табатынын түсінеді.",
        },
        cta: {
          badge: "Мәселе туралы хабарлау",
          title: "Назарды әрекетке айналдырыңыз бір басумен.",
          description:
            "Басты бет назарды iKOMEK-тің ортақ жұмыс ағымына бағыттайды.",
          button: "Өтінім жасау",
          trust: "Ортақ платформа, синхрондалған статустар, біртұтас қызмет ағымы.",
        },
        requestStatus: {
          pending: "Күтуде",
          in_progress: "Орындалуда",
          closed: "Жабық",
          open: "Ашық",
          resolved: "Шешілді",
          rejected: "Қабылданбады",
        },
        sections: {
          pulse: "Қала пульсі",
          pulseDescription: "Ортақ жаңалықтар ағынынан соңғы ескертулер мен жаңартулар.",
          categories: "Танымал санаттар",
          categoriesDescription: "Қолданыстағы платформадан келетін ортақ өтінім таксономиясы.",
          journey: "Платформа қалай жұмыс істейді",
        },
      },
      auth: {
        loginTab: "Кіру",
        registerTab: "Тіркелу",
        recoverTab: "Қалпына келтіру",
        login: {
          eyebrow: "Кіру",
          title: "Бар iKOMEK аккаунтына кіріңіз",
          subtitle: "Профиль, өтінімдер және чат тарихын бір жерден көру үшін кіріңіз.",
        },
        register: {
          eyebrow: "Тіркелу",
          title: "Аккаунт жасау",
          subtitle: "Қош келдіңіз! Тіркелу үшін мәліметтеріңізді толтырыңыз.",
        },
        recovery: {
          eyebrow: "Қалпына келтіру",
          title: "Қолжетімділікті қалпына келтіру",
          subtitle: "Аккаунтыңызға қолжетімділікті қалпына келтіру үшін мәліметтеріңізді енгізіңіз.",
        },
        verification: {
          eyebrow: "Растау",
          title: "Email мекенжайын растаңыз",
          subtitle: "iKOMEK аккаунтын белсендіру үшін email-ге жіберілген бір реттік кодты енгізіңіз.",
          titleText: "Растау коды",
          instructions: "Email мекенжайыңызға жіберілген 6 таңбалы кодты енгізіңіз.",
          sentTo: "Растау коды {{email}} мекенжайына жіберілді.",
          code: "Растау коды",
          codePlaceholder: "6 таңбалы кодты енгізіңіз",
          expiresInMinutes: "Код {{minutes}} минут ішінде жарамды.",
          confirm: "Email растау",
          resend: "Кодты қайта жіберу",
          resendIn: "{{seconds}}с кейін қайта жіберу",
          editEmail: "Email өзгерту",
          codeSent: "Растау коды email мекенжайыңызға жіберілді.",
          resent: "Жаңа растау коды жіберілді.",
          verified: "Email сәтті расталды.",
          loginBlocked: "Аккаунтыңыз әлі расталмаған. Жалғастыру үшін email-дегі кодты енгізіңіз.",
        },
        feedback: {
          loginSuccess: "Сәтті кірдіңіз",
        },
        clerk: {
          title: "Жылдам кіру",
          subtitle: "Жылдам кіру үшін Google қолданыңыз немесе төменде iKOMEK email не телефонымен жалғастырыңыз.",
          setupRequired: "Google арқылы кіру уақытша қолжетімсіз. Төменде iKOMEK аккаунты арқылы кіре аласыз.",
          google: "Google арқылы кіру",
          orLegacy: "немесе iKOMEK аккаунтын қолданыңыз",
          signedInTitle: "Google арқылы кіру аяқталды",
          signedInHint: "Профиліңізді, өтінімдеріңізді және чат тарихын дайындап жатырмыз.",
          syncing: "Аккаунтыңыз ашылып жатыр...",
          localPasswordTitle: "iKOMEK құпиясөзін жасаңыз",
          localPasswordHint: "Email немесе телефон арқылы да кіру үшін бөлек iKOMEK құпиясөзін орнатыңыз.",
          localPassword: "Жаңа құпиясөз",
          localPasswordConfirm: "Құпиясөзді қайталаңыз",
          localPasswordSave: "Құпиясөзді сақтап, жалғастыру",
          localPasswordSaved: "Құпиясөз сақталды. Енді iKOMEK аккаунты арқылы кіре аласыз.",
          localPasswordFillAll: "Құпиясөздің екі өрісін де толтырыңыз",
          localPasswordMin: "Құпиясөз кемінде 6 таңба болуы керек",
          localPasswordMismatch: "Құпиясөздер сәйкес келмейді",
          missingToken: "Кіруді растау мүмкін болмады. Қайталап көріңіз.",
          linkSecretRequired: "Бұл email iKOMEK ішінде қолданылып тұр. Email немесе телефон арқылы кіріңіз не қолдау қызметіне хабарласыңыз.",
          genericError: "Google арқылы кіру сәтсіз болды. Қайталап көріңіз.",
        },
        recoverNotice: {
          title: "Құпиясөзді қалпына келтіру әзірге қолжетімсіз",
          body: "Ортақ серверлік бөлік қазір құпиясөзді қалпына келтіру сценарийін ұсынбайды.",
          hint: "Демо-аккаунтты пайдалана аласыз немесе осы беттен жаңа аккаунт жасай аласыз.",
        },
        fields: {
          name: "Аты-жөні",
          email: "Email",
          emailOrPhone: "Email немесе телефон",
          password: "Құпиясөз",
          phone: "Телефон",
          language: "Қалаулы тіл",
          required: "Міндетті",
          optional: "Міндетті емес",
        },
      },
      dashboard: {
        title: "Тұрғынның басқару орталығы",
        description:
          "Өтінімдер, сақталған орындар, ескертулер және жеке белсенділікті бір жерден бақылаңыз.",
        cards: {
          total: "Барлық өтінім",
          active: "Белсенді",
          pending: "Күтуде",
          saved: "Сақталған орындар",
        },
      },
      requests: {
        title: "Өтініштер",
        eyebrow: "Өтініштердің бірыңғай орталығы",
        description: "Барлық ашық қалалық өтініштерді және жеке өтініштеріңізді бір жинақы жұмыс кеңістігінде бақылаңыз.",
        heroLabel: "Экранда",
        publicList: "Барлық пайдаланушылар",
        personalList: "Жеке өтініштер",
        scopeLabel: "Өтініштер тізімінің бөлімі",
        scopeAll: "Барлық өтініштер",
        scopeMine: "Менің өтініштерім",
        statusFilterLabel: "Статус сүзгісі",
        statsTotal: "Барлық өтініштер",
        searchPlaceholder: "Тақырып, мекенжай, автор немесе ID бойынша іздеу",
        listTitle: "{{count}} өтініш",
        listHint: "Карточкалар таңдалған бөлім бойынша бөлініп, іздеу, санат, басымдық және статус арқылы жаңарады.",
        requestId: "Өтініш ID",
        author: "Автор",
        unknownAuthor: "Автор көрсетілмеген",
        moreFilters: "Сүзгілер",
        filterCategory: "Санат",
        allCategories: "Барлық санаттар",
        filterPriority: "Басымдық",
        allPriorities: "Барлық басымдықтар",
        sortLabel: "Сұрыптау",
        sortNewest: "Алдымен жаңалары",
        sortOldest: "Алдымен ескілері",
        sortPriority: "Алдымен маңыздысы",
        paginationLabel: "Өтініштер беттері",
        paginationPrevious: "Артқа",
        paginationNext: "Жалғастыру",
        filtersAll: "Барлығы",
        filtersPending: "Күтуде",
        filtersProgress: "Орындалуда",
        filtersClosed: "Жабық",
        timeline: "Статус хронологиясы",
        attachments: "Тіркемелер",
        empty: "Бұл фильтр бойынша ештеңе табылмады.",
        statuses: {
          pending: "Күтуде",
          in_progress: "Жұмыста",
          closed: "Жабылды",
          open: "Ашық",
          resolved: "Шешілді",
          rejected: "Қабылданбады",
        },
        priorities: {
          unset: "Белгіленбеген",
          low: "Төмен",
          medium: "Орташа",
          high: "Жоғары",
        },
        attachmentTypes: {
          image: "Сурет",
          document: "Құжат",
        },
      },
      newRequest: {
        title: "Жаңа қалалық мәселе жіберу",
        description:
          "Орынды көрсетіп, мәселе түрін таңдап, мәліметтерді қосып, өтінімді ыңғайлы қалалық сценариймен жіберіңіз.",
        address: "Мекенжай",
        place: "Орын",
        category: "Санат",
        reason: "Себеп",
        descriptionLabel: "Сипаттама",
        attachments: "Тіркемелер",
        geolocate: "Менің орнымды пайдалану",
        summary: "Жіберу алдындағы шолу",
        locationRequired: "Жіберер алдында Астана ішіндегі координаттарды таңдаңыз.",
        invalidCoordinates: "Енгізілген ендік пен бойлық мәндері қате.",
        outOfZone: "Өтініштерді тек Астана ішінде және қаладан 15 км радиуста ғана жасауға болады.",
        zoneHint: "Астана орталығына дейінгі қашықтық: {{distance}} км. Рұқсат етілген радиус: 15 км.",
        progressLabel: "Өтінімді толтыру барысы",
        optional: "Міндетті емес",
        success: "Өтінім сәтті жіберілді.",
        geolocateUnavailable: "Бұл браузерде геолокация қолжетімсіз.",
        geolocateError: "Ағымдағы орынды анықтау мүмкін болмады.",
        addressPlaceholder: "Көше, бағдар, аула немесе кіреберіс",
        addressSearchHint: "Астанадағы мекенжайды жаза бастаңыз. Біз нүктені картаға автоматты түрде қоюға тырысамыз.",
        addressSearchLoading: "Мекенжай ізделіп, картадағы нүкте жаңартылуда…",
        addressSearchNoResults: "Бұл мекенжайды нақты табу мүмкін болмады. Нүктені картада қолмен көрсетіңіз.",
        addressSearchError: "Мекенжай іздеу уақытша қолжетімсіз. Нүктені картада қолмен таңдауға болады.",
        addressSearchResolved: "Картадағы ең жақын сәйкестік: {{address}}",
        latitude: "Ендік",
        longitude: "Бойлық",
        advancedCoordinates: "Координаттарды қолмен өзгерту",
        locationMapTitle: "Картадағы нүктені тексеріңіз",
        locationMapDescription: "Карта жүйенің мекенжайды қалай түсінгенін көрсетеді. Егер нүкте дәл болмаса, маркерді сүйреңіз немесе картаға басыңыз.",
        locationMapHint: "Үйді, ауланы немесе кіреберісті дәлірек көрсету үшін картаға басыңыз немесе маркерді сүйреңіз.",
        locationMapFallback: "Осы нүкте бойынша мекенжайды дәл анықтау мүмкін болмады. Маркерді жылжытыңыз немесе мекенжайды қолмен нақтылаңыз.",
        locationTitle: "Мәселе қай жерде болды?",
        locationDescription: "Алдымен орынды көрсетіңіз, сонда жүйе қай қызметті жіберу керегін түсінеді.",
        typeTitle: "Не болды?",
        typeDescription: "Алдымен санатты, содан кейін себеп пен нақты орынды таңдаңыз.",
        descriptionTitle: "Жағдайды сипаттаңыз",
        descriptionDescription: "Қысқа әрі түсінікті сипаттама операторға өтінімді жылдамырақ өңдеуге көмектеседі.",
        descriptionPlaceholder: "Не болды, оны қашан байқадыңыз және топ нені ескеруі керек?",
        descriptionHelper: "Қажет болса, шұғылдықты, көрінетін зақымды, бағдарларды немесе кіру ерекшеліктерін жазыңыз.",
        attachmentsTitle: "Фото және баптаулар",
        attachmentsDescription: "Фото міндетті емес, бірақ операторға жағдайды алдын ала түсінуге көмектеседі.",
        uploadTitle: "Файлдарды осында сүйреп әкеліңіз немесе құрылғыдан жүктеңіз",
        uploadDescription: "Мәселенің, айналаның немесе бағдардың анық фотолары жақсы көмектеседі.",
        chooseFiles: "Файл таңдау",
        removeFile: "Өшіру",
        attachmentsCount_one: "{{count}} файл",
        attachmentsCount_other: "{{count}} файл",
        visibilityTitle: "Көрінуі",
        visibilityDescription: "Бұл өтінім басқа тұрғындарға ашық тізімде көріне ме, соны таңдаңыз.",
        visibilityPublic: "Барлығына көрінеді",
        visibilityPrivate: "Тек сізге және операторларға көрінеді",
        savedPlacesTitle: "Сақталған орындар",
        savedPlacesDescription: "Жылдам бастау үшін сақталған мекенжайды таңдаңыз немесе орынды қолмен енгізіңіз.",
        saveCurrentLocation: "Бұл мекенжайды сақтау",
        saveCurrentLocationHint: "Келесі өтініштер үшін таңдалған нүктені профильге қосыңыз.",
        locationSaved: "Мекенжай профильге сақталды",
        manualLocationTitle: "Қолмен енгізу",
        manualLocationDescription: "Мекенжай мен координаттарды өзіңіз толтыру",
        categoryTitle: "Санатты таңдаңыз",
        categoryDescription: "Мәселе қай қалалық қызметке қатысты екенін көрсетіңіз.",
        reasonTitle: "Себебін таңдаңыз",
        reasonDescription: "Себептер таңдалған санатқа байланысты көрсетіледі.",
        reasonPlaceholderHint: "Сәйкес себептерді көру үшін алдымен санатты таңдаңыз.",
        placeTitle: "Нақты қай жерде?",
        placeDescription: "Бұл қадам mobile flow логикасын сақтап, өтінімді дұрыс қызметке жібереді.",
        placePlaceholder: "Мысалы: аула, кіреберіс, тұрақ, көше",
        placeHelper: "Дайын нұсқаны таңдауға да, орынды өзіңіз нақтылауға да болады.",
        selectCategoryFirst: "Себепті таңдау үшін алдымен санатты таңдаңыз.",
        reviewTitle: "Жіберер алдында тексеріңіз",
        reviewDescription: "Барлығы дұрыс толтырылғанына көз жеткізіңіз. Жіберілгеннен кейін операторлар бұл деректерді web пен mobile-де бірдей көреді.",
        reviewReady: "Барлық міндетті өріс толтырылған. Өтінімді жіберуге болады.",
        reviewPending: "Әлі {{count}} міндетті өрісті тексеру керек.",
        back: "Артқа",
        next: "Келесі қадам",
        continueToReview: "Тексеруге өту",
        descriptionTooShort: "Сипаттама кемінде {{count}} таңбадан тұруы керек.",
        submit: "Өтінім жіберу",
        summaryTitle: "Өтінімнің тірі шолуы",
        summaryDescription: "Бұл панель форма толтырылған сайын жаңарып, жіберер алдында жетіспейтін жерлерді көруге көмектеседі.",
        summaryReadyDescription: "Барлық міндетті дерек дайын. Өтінімді қазір жіберуге болады.",
        summaryStatusReady: "Жіберуге дайын",
        summaryStatusDraft: "Толықтыру керек",
        summaryMapTitle: "Орналасу көрінісі",
        summaryMapEmpty: "Картада нүктені көру үшін дұрыс координаттарды қосыңыз.",
        sectionStatusReady: "Дайын",
        sectionStatusNeedsAttention: "Орынды тексеріңіз",
        missingFieldsTitle: "Жетпейтіні",
        missingFields: {
          address: "Мекенжай қажет",
          location: "Астана аймағындағы дұрыс нүкте қажет",
          category: "Санатты таңдаңыз",
          reason: "Себепті таңдаңыз",
          place: "Нақты орынды көрсетіңіз",
          description: "Қысқаша сипаттама қосыңыз",
        },
        summaryItems: {
          address: "Мекенжай",
          place: "Орын",
          category: "Санат",
          reason: "Себеп",
          files: "Файлдар",
          visibility: "Көрінуі",
          coordinates: "Координаттар",
        },
        steps: {
          location: "Орын",
          type: "Түрі",
          description: "Сипаттама",
          attachments: "Медиа",
          review: "Тексеру",
        },
        stepDescriptions: {
          location: "Астана ішіндегі нүктені растаңыз",
          type: "Санат, себеп және орынды таңдаңыз",
          description: "Операторға контекст қосыңыз",
          attachments: "Фото және көріну баптауы",
          review: "Жіберер алдындағы дайындықты тексеріңіз",
        },
        stepErrors: {
          location: "Мекенжайды қосып, Астана аймағындағы дұрыс нүктені таңдаңыз.",
          type: "Жалғастырмас бұрын санат пен себепті таңдаңыз.",
          descriptionRequired: "Жалғастырмас бұрын қысқаша сипаттама қосыңыз.",
          descriptionLength: "Сипаттама кемінде {{count}} таңбадан тұруы керек.",
          review: "Өтінімді жібермес бұрын барлық міндетті өрістерді толтырыңыз.",
        },
        placeOptions: {
          apartment: "Пәтер",
          house: "Үй",
          office: "Кеңсе",
          street: "Көше",
          park: "Саябақ",
          other: "Басқа",
        },
      },
      requestDetails: {
        overview: "Шолу",
        history: "Тарих",
        messages: "Хабарламалар",
        status: "Статус",
        created: "Жасалды",
        category: "Санат",
        reason: "Себеп",
        location: "Картадағы орын",
        openChat: "Чатты ашу",
        chatPreview: "Чат үзіндісі",
        noTimeline: "Статус тарихы әзірге бос.",
        noAttachments: "Әзірге тіркеме жоқ.",
        noMessages: "Әзірге хабарлама жоқ.",
        notFoundTitle: "Өтініш табылмады",
        notFoundDescription: "Өтінішті серверден жүктеу мүмкін болмады.",
      },
      chat: {
        title: "Өтінім чаты",
        description: "Хабарламалар тұрғын мен оператор арасында нақты уақытта синхрондалады.",
        placeholder: "Хабарлама жазыңыз",
        message: "Хабарлама",
        sent: "Хабарлама жіберілді",
        requestLabel: "Өтінім",
        live: "Онлайн",
        reconnecting: "Қосылуда...",
        attachment: "Тіркеме",
        attachMedia: "Медиа тіркеу",
        emptyTitle: "Әзірге хабарлама жоқ",
        emptyDescription: "Оператормен диалогты осы жерден бастаңыз.",
        unavailableTitle: "Чат қолжетімсіз",
        unavailableDescription: "Бұл чатты тек өтінім авторы, оператор немесе әкімші қолдана алады.",
      },
      notFound: {
        title: "Бет табылмады",
        description: "Бұл маршрут iKOMEK жұмыс кеңістігіне кірмейді.",
      },
      aiAssistant: {
        label: "AI ассистент",
        title: "AI ассистент",
        subtitle: "Өтінімдер, мәртебелер және навигация бойынша көмектеседі",
        greeting: "Сәлеметсіз бе! Мен iKOMEK 109, өтінімдер, жаңалықтар және мәртебелер бойынша көмектесемін.",
        placeholder: "Сұрағыңызды жазыңыз...",
        empty: "Өтінім жасау немесе мәртебені көру туралы сұраңыз.",
        configured: "Gemini API key бапталмаған",
        close: "AI ассистентті жабу",
        failed: "Жауап алу мүмкін болмады. Backend пен AI баптауларын тексеріңіз.",
      },
      news: {
        title: "Қалалық жаңалықтар мен хабарламалар",
        description: "Қолданыстағы қалалық контент ағынына арналған ұқыпты редакциялық интерфейс.",
        searchPlaceholder: "Тақырып немесе мәтін бойынша іздеу",
        allCategories: "Барлық санаттар",
        allTypes: "Барлық түрлер",
        periodAll: "Барлық кезеңдер",
        periodActive: "Белсенді",
        periodFinished: "Аяқталған",
        periodNoPeriod: "Кезеңсіз",
        sortNewest: "Алдымен жаңалары",
        sortOldest: "Алдымен ескілері",
        noResults: "Ештеңе табылмады",
        emptyFiltered: "Сүзгілерді немесе іздеу сұрауын өзгертіңіз.",
        modalTitle: "Жаңалық",
        createdAgo: "{{count}} күн бұрын жасалды",
        createdHoursAgo: "{{count}} сағат бұрын жасалды",
        createdJustNow: "Жаңа ғана жасалды",
        period: "КЕЗЕҢ",
        location: "ОРНАЛАСУЫ",
        types: {
          emergency: "Авариялық жұмыстар",
          weather: "Ауа райы жағдайлары",
          planned: "Жоспарлы жұмыстар",
          road: "Жол жағдайлары",
          education: "Білім басқармасы",
          events: "Қала іс-шаралары",
        },
        categories: {
          roads: "Жолдар",
          utilities: "Коммуналдық қызметтер",
          transport: "Көлік",
          education: "Білім",
          weather: "Ауа райы",
          improvement: "Абаттандыру",
        },
        translationStatus: {
          translated: "Аударылды",
          skipped: "Аударылмаған",
          failed: "Аударма қатесі",
        },
      },
      categories: {
        electricity: "Электр қуаты",
        water: "Сумен қамтамасыз ету",
        heating: "Жылыту",
        publicOrder: "Тәртіп бұзушылық",
        sewage: "Кәріз",
        waste: "Қоқыс",
        roads: "Жолдар",
        streetLighting: "Көше жарығы",
        other: "Басқа",
      },
      placeTypes: {
        apartment: "Пәтер",
        house: "Жеке үй",
        office: "Кеңсе",
        street: "Көше",
        park: "Саябақ/алаң",
        entrance: "Кіреберіс",
        yard: "Аула",
        parking: "Тұрақ",
        other: "Басқа",
      },
      problemTypes: {
        electricity: {
          power_outage: "Электр қуаты жоқ",
          voltage_issue: "Кернеу ауытқуы",
          damaged_cables: "Зақымдалған кабельдер",
          street_light: "Көше шамы істемейді",
          sparks: "Ұшқын немесе өрт қаупі",
          other: "Электрге қатысты басқа мәселе",
        },
        water: {
          no_water: "Су жоқ",
          low_pressure: "Су қысымы төмен",
          pipe_leak: "Құбыр ағып тұр",
          dirty_water: "Лас немесе лай су",
          sewage: "Кәріз мәселесі",
          other: "Суға қатысты басқа мәселе",
        },
        sewage: {
          blockage: "Кәріз бітелуі",
          leak: "Кәріз ағуы",
          odor: "Кәріздің жағымсыз иісі",
          overflow: "Кәріздің тасуы",
          manhole: "Ашық немесе зақымдалған люк",
          other: "Кәрізге қатысты басқа мәселе",
        },
        roads: {
          pothole: "Жолдағы шұңқыр",
          damaged_pavement: "Зақымдалған жабын",
          road_sign: "Жол белгісі жоқ немесе зақымдалған",
          traffic_light: "Бағдаршам мәселесі",
          road_marking: "Жол таңбасы қажет",
          other: "Жолға қатысты басқа мәселе",
        },
        public_order: {
          noise: "Шу туралы шағым",
          illegal_parking: "Дұрыс емес тұрақ",
          vandalism: "Вандализм",
          abandoned_vehicle: "Тасталған көлік",
          stray_animals: "Қаңғыбас жануарлар",
          other: "Тәртіпке қатысты басқа мәселе",
        },
        waste: {
          overflowing: "Толып тұрған қоқыс контейнері",
          illegal_dump: "Заңсыз қоқыс орны",
          missed_collection: "Қоқыс шығарылмаған",
          hazardous: "Қауіпті қалдықтар",
          bulk_waste: "Ірі көлемді қоқысты шығару керек",
          other: "Қоқысқа қатысты басқа мәселе",
        },
        heating: {
          no_heating: "Жылыту жоқ",
          radiator_leak: "Радиатор ағып тұр",
          cold_apartment: "Пәтер суық",
          overheating: "Тым ыстық",
          noise: "Жылыту жүйесіндегі шу",
          other: "Жылытуға қатысты басқа мәселе",
        },
        street_lighting: {
          lamp_out: "Шам жанбайды",
          flickering: "Шам жыпылықтайды",
          damaged_pole: "Бағана зақымдалған",
          dark_area: "Нашар жарықтанған аймақ",
          timer: "Таймер ақауы",
          other: "Жарыққа қатысты басқа мәселе",
        },
        other: {
          general: "Жалпы шағым",
          suggestion: "Ұсыныс",
          question: "Сұрақ",
          other: "Басқа",
        },
      },
      reasons: {
        electricity: {
          infrastructure: "Инфрақұрылым ақауы",
          weather: "Ауа райынан болған зақым",
          overload: "Жүйе шамадан тыс жүктелген",
          maintenance: "Қызмет көрсету қажет",
          accident: "Апат немесе сыртқы зақым",
          unknown: "Себебі белгісіз",
        },
        water: {
          pipe_burst: "Құбыр жарылған",
          maintenance: "Жоспарлы жұмыс",
          infrastructure: "Тозған инфрақұрылым",
          external: "Сыртқы зақым",
          pressure: "Қысым мәселесі",
          unknown: "Себебі белгісіз",
        },
        sewage: {
          pipe: "Құбыр мәселесі",
          blockage: "Бітелу",
          wear: "Құбырдың тозуы",
          overflow: "Тасу",
          odor: "Иіс мәселесі",
          infrastructure: "Тозған инфрақұрылым",
          rain: "Қатты жаңбыр",
          maintenance: "Қызмет көрсету қажет",
          unknown: "Себебі белгісіз",
        },
        roads: {
          weather: "Ауа райынан тозу",
          traffic: "Көлік әсерінен зақым",
          construction: "Құрылыстан зақым",
          age: "Уақыт өте тозу",
          accident: "Апаттан кейінгі зақым",
          unknown: "Себебі белгісіз",
        },
        public_order: {
          resident: "Тұрғын шағымы",
          safety: "Қоғамдық қауіпсіздік мәселесі",
          community: "Аудан мәселесі",
          legal: "Заң бұзушылық",
          recurring: "Қайталанатын мәселе",
          other: "Басқа себеп",
        },
        waste: {
          schedule: "Кесте мәселесі",
          container: "Контейнердің зақымы",
          illegal: "Заңсыз тастау",
          volume: "Көлемнің артуы",
          access: "Қол жеткізу мәселесі",
          other: "Басқа себеп",
        },
        heating: {
          boiler: "Қазандық мәселесі",
          pipe: "Құбыр мәселесі",
          system: "Жүйе ақауы",
          regulation: "Температураны реттеу",
          maintenance: "Қызмет көрсету қажет",
          unknown: "Себебі белгісіз",
        },
        street_lighting: {
          bulb: "Шамның жануы",
          electrical: "Электр ақауы",
          vandalism: "Вандализм",
          timer: "Таймер ақауы",
          age: "Уақыт өте тозу",
          unknown: "Себебі белгісіз",
        },
        other: {
          quality: "Қызмет сапасы",
          safety: "Қауіпсіздік мәселесі",
          environment: "Экологиялық мәселе",
          other: "Басқа",
        },
      },
      map: {
        title: "Қаланың тірі картасы",
        subtitle: "Астана · нақты уақыттағы өтініштер",
        subtitleWithCount: "Астана · нақты уақыттағы {{count}} өтініш",
        description: "Кластерленген ашық өтінімдерді, heatmap аймақтарын және аудан белсенділігін зерттеңіз.",
        all: "Барлық өтінім",
        mine: "Менің өтінімдерім",
        heatmap: "Жылу картасы",
        filters: {
          allRequests: "Барлық өтініштер",
          myRequests: "Менің өтініштерім",
          heatmap: "Жылу картасы",
          allCategories: "Барлық санаттар",
          allStatuses: "Барлық статустар",
          allPriorities: "Барлық басымдықтар",
          today: "Бүгін",
          week: "Апта",
          month: "Ай",
          dateRangeHelp: "Қатарынан 7 күнге дейін таңдаңыз",
          previousMonth: "Алдыңғы ай",
          nextMonth: "Келесі ай",
          reset: "Сүзгілерді қалпына келтіру",
          showFilters: "Сүзгілерді көрсету",
          hideFilters: "Сүзгілерді жасыру",
        },
        analytics: {
          title: "Қала аналитикасы",
          density: "Аудандар бойынша тығыздық",
          categories: "Танымал санаттар",
          timeline: "Айлар бойынша динамика",
          timelineMonths: "Айлар бойынша динамика",
          timelineWeekdays: "Апта күндері бойынша динамика",
          timelineHours: "Сағаттар бойынша динамика",
          timelineView: "График түрі",
          timelineSelectedPeriod: "Таңдалған кезең",
          timelineAllMonths: "Барлық айлар",
          chartTotal: "Барлығы",
          hotspots: "Үздік проблемалы орындар",
          activity: "Күндер мен сағаттар бойынша белсенділік",
          otherDistrict: "Басқа",
          seriesAll: "Барлық өтініштер",
          weekdays: {
            mon: "Дс",
            tue: "Сс",
            wed: "Ср",
            thu: "Бс",
            fri: "Жм",
            sat: "Сн",
            sun: "Жк",
          },
          requestsCount: "{{count}} өтініш",
          insufficientData: "Аналитика үшін деректер жеткіліксіз",
        },
      },
      profile: {
        title: "Профиль және сақталған орындар",
        addLocation: "Орын қосу",
        notifications: "Хабарламалар",
      },
      savedLocationTypes: {
        home: "Үй",
        work: "Жұмыс",
        study: "Оқу",
        family: "Отбасы",
        other: "Басқа",
      },
      cabinet: {
        title: "Жеке кабинет",
        memberSince: "Тұрғын болғалы",
        editProfile: "Профильді өңдеу",
        logout: "Шығу",
        comingSoon: "Жақында қолжетімді болады",
        profileEditor: {
          title: "Сіздің деректеріңіз",
          displayName: "Сізге қалай жүгінейік?",
          displayNamePlaceholder: "Жүгіну аты",
          displayNameHint: "Бұл ат пікірлеріңіз бен комментарийлеріңізде көрсетіледі.",
          changePhoto: "Фотоны өзгерту",
          removePhoto: "Фотоны жою",
          cropPhoto: "Фотоны қиып алу",
          photoScale: "Фото өлшемі",
          photoError: "Фотоны дайындау мүмкін болмады",
          personalData: "Жеке деректер",
          firstName: "Аты",
          lastName: "Тегі",
          gender: "Жынысы",
          birthDate: "Туған күні",
          phone: "Телефон",
          language: "Интерфейс тілі",
          notifications: "Қалалық хабарламаларды алу",
          save: "Сақтау",
          saved: "Профиль жаңартылды",
          nameRequired: "Атыңызды және тегіңізді енгізіңіз",
          nameLettersOnly: "Аты мен тегінде тек әріптерді қолданыңыз, дефис тек атау бөліктерінің арасында болуы керек",
          phoneInvalid: "Қазақстан нөмірін 7-ден басталатын 11 цифрмен енгізіңіз",
          birthDateInvalid: "Болашақ емес, дұрыс туған күнді енгізіңіз",
          genders: {
            male: "Е",
            female: "Ә",
          },
        },
        stats: {
          title: "Менің статистикам",
          total: "Барлық өтініштер",
          closed: "Жабылды",
          inProgress: "Жұмыста",
          pending: "Күтуде",
          avgDays: "Орташа жабылу уақыты",
          days: "күн",
        },
        activity: {
          title: "Айлар бойынша белсенділік",
        },
        recent: {
          title: "Соңғы өтініштер",
          all: "Барлық өтініштер",
          category: "Санат",
        },
        saved: {
          title: "Сақталған орындар",
          empty: "Сақталған мекенжайлар қосылғаннан кейін осында пайда болады.",
          add: "Мекенжай қосу",
          save: "Мекенжайды сақтау",
          created: "Мекенжай сақталды",
          deleted: "Мекенжай жойылды",
          label: "Атауы",
          labelPlaceholder: "Мысалы: Үй",
          type: "Орын түрі",
          address: "Мекенжай",
          addressPlaceholder: "Көше, үй, кіреберіс",
          latitude: "Ендік",
          longitude: "Бойлық",
          coordinatesHint: "Координаттар міндетті емес. Бос қалса, мекенжайды автоматты түрде табуға тырысамыз.",
          formRequired: "Атауы мен мекенжайын толтырыңыз.",
          addressNotFound: "Нақты мекенжай табылмады. Нүктені картадан нақтылаңыз.",
          outOfZone: "Сақталған мекенжайларды тек Астана ішінде және қаладан 15 км радиуста ғана қосуға болады. Қазір орталыққа дейін: {{distance}} км.",
          findOnMap: "Картадан табу",
          findingAddress: "Мекенжай ізделуде...",
          resolvingAddress: "Мекенжай анықталуда...",
          mapTitle: "Нүктені картадан нақтылаңыз",
          mapHint: "Мекенжай дұрыс анықталмаса, картаны басыңыз немесе маркерді қажетті орынға сүйреңіз.",
          mapFallback: "Мекенжай автоматты түрде табылмады. Нүктені картадан қолмен таңдаңыз.",
          approximateFound: "Біз шамамен нүктені таптық. Сақтамас бұрын картадан тексеріңіз.",
        },
        alerts: {
          title: "Ескертулер мен хабарламалар",
          empty: "Әзірге ескертулер немесе хабарламалар жоқ.",
        },
      },
      settings: {
        title: "Баптаулар",
        subtitle: "Профильді, құпиясөзді және интерфейс тілін бір жерден басқарыңыз.",
        profileTitle: "Профиль",
        profileDescription: "Жеке деректер мен профиль фотосын өзгертіңіз.",
        editProfile: "Профильді өңдеу",
        passwordTitle: "Құпиясөзді ауыстыру",
        passwordDescription: "Күшті құпиясөз қолданыңыз және оны ешкімге айтпаңыз.",
        passwordSetupDescription: "Email немесе телефон арқылы iKOMEK аккаунтына кіру үшін құпиясөз жасаңыз.",
        currentPassword: "Ескі құпиясөзді енгізіңіз",
        newPassword: "Жаңа құпиясөзді енгізіңіз",
        confirmPassword: "Жаңа құпиясөзді қайталаңыз",
        savePassword: "Құпиясөзді сақтау",
        passwordSaved: "Құпиясөз жаңартылды",
        passwordFillAll: "Құпиясөз өрістерінің бәрін толтырыңыз",
        passwordMin: "Жаңа құпиясөз кемінде 6 таңба болуы керек",
        passwordMismatch: "Жаңа құпиясөздер сәйкес келмейді",
        languageTitle: "Интерфейс тілі",
        languageDescription: "Платформада қолданылатын тілді таңдаңыз.",
        saveLanguage: "Тілді сақтау",
        languageSaved: "Тіл жаңартылды",
      },
      operator: {
        title: "Оператордың жұмыс орны",
        eyebrow: "Операциялық кезек",
        description:
          "Кезекті басқарыңыз, деректерді тексеріңіз және статустарды ортақ жүйеден шықпай жаңартыңыз.",
        update: "Статусты жаңарту",
        details: "Детальдар",
        priority: "Басымдық",
        visibleNow: "Экранда",
        queueLabel: "Оператор кезегі",
        searchPlaceholder: "Тақырып, мекенжай, автор, санат немесе ID бойынша іздеу",
        listTitle: "Кезекте {{count}} өтініш",
        listHint: "Карточкалар іздеу, статус, санат, басымдық және сұрыптау арқылы жаңарады.",
        empty: "Бұл оператор сүзгісі бойынша ештеңе табылмады.",
        department: "Бөлім",
        departmentPlaceholder: "Жауапты бөлім",
        internalNote: "Ішкі ескертпе",
        internalNotePlaceholder: "Тек операторлар мен әкімшілерге көрінеді",
        closeComment: "Жабу түсініктемесі",
        closeCommentPlaceholder: "Өтінішті жабу алдында не жасалғанын сипаттаңыз",
        closeCommentHelper: "Статус жабылған кезде міндетті.",
        closeCommentOptional: "Өтінішті жапқанда толтырыңыз.",
        closeCommentRequired: "Өтінішті жаппас бұрын түсініктеме қосыңыз.",
        statusPriorityRequired: "Өтінішті жұмысқа ауыстыру немесе жабу алдында басымдықты таңдаңыз.",
        noRequestSelected: "Алдымен өтінішті таңдаңыз.",
        saveChanges: "Өзгерістерді сақтау",
        updateSuccess: "Өтініш жаңартылды",
        stats: {
          title: "Менің оператор статистикам",
          totalQueue: "Кезектің барлығы",
          assigned: "Жұмысқа алынды",
          closed: "Жабылды",
          inProgress: "Қазір жұмыста",
          pending: "Күтуде",
          queue: "Кезек күтуде",
          avgDays: "Орташа жабылу уақыты",
          days: "күн",
        },
        activity: {
          title: "Айлар бойынша жүктемем",
        },
        recent: {
          title: "Соңғы өңделген өтініштер",
        },
      },
      admin: {
        title: "Әкімші студиясы",
        description: "Метрикаларды бақылап, қалалық жаңартуларды жинақы жұмыс процесі арқылы жариялаңыз.",
        publish: "Жаңалық жариялау",
        stats: {
          title: "Платформа статистикасы",
          totalRequests: "Барлық өтінімдер",
          activeRequests: "Белсенді өтінімдер",
          pendingRequests: "Күтудегі өтінімдер",
          citizens: "Тұрғындар",
          operators: "Операторлар",
          closed: "Жабылды",
          topCategory: "Үздік санат",
        },
        activity: {
          title: "Платформа белсенділігі",
        },
        categories: {
          title: "Үздік санаттар",
        },
        workload: {
          title: "Операторлар жүктемесі",
          operator: "Оператор",
          inProgress: "Жұмыста",
          closed: "Жабылды",
          total: "Барлығы",
        },
        news: {
          created: "Жаңалық жарияланды.",
          updated: "Жаңалық жаңартылды.",
          deleted: "Жаңалық жойылды.",
          fillTitleAndText: "Жаңалықтың тақырыбы мен мәтінін толтырыңыз.",
          selectOneType: "Кемінде бір жаңалық түрін таңдаңыз.",
          searchPlaceholder: "Тақырып немесе мәтін бойынша іздеу",
          allCategories: "Барлық санаттар",
          allTypes: "Барлық түрлер",
          periodAll: "Барлық кезеңдер",
          periodActive: "Белсенді",
          periodFinished: "Аяқталған",
          periodNoPeriod: "Кезеңсіз",
          sortNewest: "Алдымен жаңалары",
          sortOldest: "Алдымен ескілері",
          editorDescription: "Жаңалықты жасаңыз немесе жаңартыңыз, содан кейін қажет болса аудармаларын түзетіңіз.",
          titleLabel: "Жаңалық тақырыбы",
          contentLabel: "Жаңалық мәтіні",
          summaryLabel: "Қысқаша сипаттама",
          categoryLabel: "Санат",
          typesLabel: "Түрлер",
          locationLabel: "Орналасуы",
          periodStartLabel: "Кезеңнің басталуы",
          periodEndLabel: "Кезеңнің аяқталуы",
          translate: "Аудару",
          retranslate: "Қайта аудару",
          publishWithoutTranslation: "Аудармасыз жариялау",
          localizedTitleLabel: "Локализацияланған тақырып {{lang}}",
          localizedContentLabel: "Локализацияланған мәтін {{lang}}",
          localizedSummaryLabel: "Локализацияланған қысқаша сипаттама {{lang}}",
          editArticleTitle: "Жаңалықты өңдеу",
        },
      },
      emptyStates: {
        genericTitle: "Көрсететін дерек жоқ",
        genericDescription: "Backend жазбаларды қайтарған кезде олар автоматты түрде осында шығады.",
      },
    },
  },
} as const;

function detectLanguage() {
  const stored = session.getLocale();
  if (stored === "ru" || stored === "kz" || stored === "en") {
    return stored;
  }

  if (typeof navigator === "undefined") {
    return "en";
  }

  if (navigator.language.startsWith("ru")) {
    return "ru";
  }

  if (navigator.language.startsWith("kk") || navigator.language.startsWith("kz")) {
    return "kz";
  }

  return "en";
}

if (!i18n.isInitialized) {
  void i18n.use(initReactI18next).init({
    resources,
    lng: detectLanguage(),
    fallbackLng: "en",
    interpolation: {
      escapeValue: false,
    },
  });
}

export { i18n };
