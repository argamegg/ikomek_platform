...
export default function OperatorProfile() {
  ...
  const handleLogout = () => {
    Alert.alert(t('operator.profile.logout'), t('profile.logoutConfirm'), [
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 48 }]}>
        <View style={styles.header}>
          <Text style={styles.headerTitle}>{t('operator.profile.title')}</Text>
        </View>

        <View style={styles.profileCard}>
          <View style={styles.avatar}>
            <Text style={styles.avatarText}>{getInitials(user?.full_name || 'O')}</Text>
          </View>
          <View style={styles.profileInfo}>
            <Text style={styles.profileName}>{user?.full_name}</Text>
            <Text style={styles.profileEmail}>{user?.email}</Text>
            <View style={styles.roleBadge}>
              <Text style={styles.roleText}>{t('roles.operator')}</Text>
            </View>
          </View>
        </View>

        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{t('operator.profile.settings')}</Text>
          <View style={styles.settingsCard}>
            <TouchableOpacity style={styles.settingItem} onPress={() => setIsLanguageModalOpen(true)}>
              ...
              <Text style={styles.settingText}>{t('operator.profile.language')}</Text>
        ...
        >
          ...
          <Text style={styles.logoutText}>{t('operator.profile.logout')}</Text>
        </TouchableOpacity>

        <Text style={styles.version}>iKomek 109 v2.0.0 - {t('operator.profile.panel')}</Text>
      </ScrollView>
      ...
      <Modal visible={isLanguageModalOpen} transparent animationType="slide">
        <View style={styles.langModalOverlay}>
          <View style={[styles.langModalContent, { paddingBottom: insets.bottom + 16 }]}>
            ...
            <Text style={styles.langModalTitle}>{t('operator.profile.language')}</Text>
